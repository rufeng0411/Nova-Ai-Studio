...
import type { OfficialMediaBudget } from "../../saas/media/officialMediaFallbackStateMachine.js";
import type { QualityContractMode } from "../../saas/constraints/qualityCanaryPolicy.js";
...
export type SubAgentSessionOptions = {
  ...
  /** PD-SAAS-FORK: P0-1 exact capability context inherited from the parent. */
  capabilityContext?: CapabilityBindingContext;
  capabilityCompletionMode?: CapabilityCompletionMode;
  /** PD-SAAS-FORK P0-2: parent-authenticated scope and frozen quality policy. */
  ...
  /** PD-SAAS-FORK P0-6: exact parent official-media policy and shared budget. */
  goalToolPolicy?: GoalToolPolicy;
  officialMediaBudget?: OfficialMediaBudget;
  /** PD-SAAS-FORK P0-3: parent-authenticated explicit user URLs. */
  trustedUserExplicitUrls?: string[];
  /** PD-SAAS-FORK P0-4: inherited task generation for scope-bound media. */
  sessionTaskDirectory?: SessionTaskDirectory;
  knownTaskArtifactDirs?: string[];
  /** PD-SAAS-FORK ES9 P0-F: inherit parent SDM for sequential stage gate. */
  sessionDeliverableManifest?: import("../../saas/taskState/sessionDeliverableManifest.js").SessionDeliverableManifest;
  /**
...
export class SubAgentSession {
  ...
  async run(): Promise<SubagentReport> {
    ...
    const generator = loop.run({
      ...
      capabilityContext: this.options.capabilityContext,
      capabilityCompletionMode: this.options.capabilityCompletionMode,
      trustedExecutionScope: this.options.trustedExecutionScope,
      sessionGoalQualityContract: this.options.sessionGoalQualityContract,
      qualityContractMode: this.options.qualityContractMode,
      goalToolPolicy: this.options.goalToolPolicy,
      officialMediaBudget: this.options.officialMediaBudget,
      trustedUserExplicitUrls: this.options.trustedUserExplicitUrls,
      sessionTaskDirectory: this.options.sessionTaskDirectory,
      knownTaskArtifactDirs: this.options.knownTaskArtifactDirs,
      sessionDeliverableManifest: this.options.sessionDeliverableManifest,
    });
    while (true) {
  ...
  private buildScopedRegistry(): ToolRegistry {
    ...
    const forceReadOnly = this.options.definition.isReadOnly
      ...
      || this.options.capabilityCompletionMode === "consultation";
    for (const tool of this.options.parentDependencies.tools.registry.list()) {
      ...
      if (!isToolAllowedForCapabilityCompletionMode(
        tool.name,
        this.options.capabilityCompletionMode,
      )) {
        continue;
      }
      if (!isToolAllowedByQualityContract(
        tool.name,
        this.options.sessionGoalQualityContract,
        this.options.qualityContractMode ?? "off",
      )) {
        continue;
      }
      if (!isToolAllowedByGoalToolPolicy(
        tool.name,
        tool.kind,
        this.options.goalToolPolicy,
      )) {
        continue;
      }
