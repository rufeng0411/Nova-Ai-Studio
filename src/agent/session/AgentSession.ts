...
export class AgentSession {
  ...
  async *submit(input: AgentInput, submitOptions: AgentSubmitOptions = {}): AsyncGenerator<AgentEvent, void, unknown> {
    ...
    const runResult = yield* this.options.turnRunner.run({
      ...
      // PD-SAAS-FORK: Hub「试一下」技能锁定上下文传入 TurnRunner
      capabilityContext: submitOptions.capabilityContext,
      promptLanguage: submitOptions.promptLanguage,
      // PD-SAAS-FORK (P0-7): TurnRunner validates this ref against its target transcript.
      acceptedInputRef: submitOptions.acceptedInputRef,
      acceptedInputFingerprint: submitOptions.acceptedInputFingerprint,
      acceptedInputAttachmentDescriptors: submitOptions.acceptedInputAttachmentDescriptors,
      queueItemId: submitOptions.queueItemId,
      // PD-SAAS-FORK P0-2: authenticated tenant/principal scope reaches bootstrap and tools.
      trustedExecutionScope: submitOptions.trustedExecutionScope,
