...
// PD-SAAS-FORK: outbound tool rate limiting
import { createResilienceAwareScheduler } from "../../saas/resilience/safeConcurrentScheduler.js";
...
export function createAgentSessionWithStorage(options: CreateAgentSessionOptions): {
  ...
  // PD-SAAS-FORK: wrap default scheduler with outbound gate when resilience enabled
  const resilienceConfig = resolveResilienceConfig();
  const scheduler = options.dependencies.tools.scheduler
    ?? (resilienceConfig.enabled
      ? createResilienceAwareScheduler(toolRuntime, options.dependencies.tools.registry, resilienceConfig)
