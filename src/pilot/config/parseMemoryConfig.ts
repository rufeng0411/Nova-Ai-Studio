...
import {
  ...
  type PilotMemoryProjectContinuityConfig,
...
export function parseMemoryConfig(
  ...
  const projectContinuity = parseMemoryProjectContinuity(rawMemory.projectContinuity, diagnostics);
...
function parseMemoryProjectContinuity(
  value: unknown,
  diagnostics: PilotConfigDiagnostic[],
): PilotMemoryProjectContinuityConfig | undefined {
  if (value === undefined) {
    return { enabled: true };
  }
  if (!isRecord(value)) {
    throw new PilotConfigError("CONFIG_MEMORY_VALUE_INVALID", "memory.projectContinuity must be an object.");
  }
  const continuity: PilotMemoryProjectContinuityConfig = {
    enabled: readBoolean(value.enabled, true, "memory.projectContinuity.enabled"),
  };
  for (const key of Object.keys(value)) {
    if (key !== "enabled") {
      diagnostics.push({
        code: "CONFIG_MEMORY_UNKNOWN_FIELD",
        severity: "warning",
        message: `Unknown memory.projectContinuity field ${key}.`,
        path: `memory.projectContinuity.${key}`,
        recoverable: true,
      });
    }
  }
