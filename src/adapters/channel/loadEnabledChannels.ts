...
const IM_CHAT_KEYS = new Set(["wecom", "dingtalk", "whatsapp", "wecomCallback"]);

/** PD-SAAS-FORK: SaaS default — block IM chat adapters unless PILOTDECK_IM_CHANNELS allows. */
function isImChatPlatformAllowed(key: string): boolean {
  if (!IM_CHAT_KEYS.has(key)) return true;
...
export async function loadEnabledChannels(adapters: PilotAdaptersConfig | undefined): Promise<ChannelAdapter[]> {
  ...
  for (const [key, loader] of Object.entries(CHANNEL_LOADERS)) {
    ...
    // PD-SAAS-FORK: platform flag gate for WeCom / DingTalk / WhatsApp chat
    if (!isImChatPlatformAllowed(key)) {
      console.warn(`[adapters] skip "${key}": PILOTDECK_IM_CHANNELS is off`);
      continue;
    }
