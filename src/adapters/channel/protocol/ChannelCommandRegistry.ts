...
const commands: ChannelCommand[] = [
...

  {
    name: "update",
    aliases: ["升级", "更新"],
    description: "Pull latest code, rebuild, and restart Nova Ai-Studio",
    systemLevel: true,
    handler: async (ctx, arg) => {
      ...
      // Execute the update
      await ctx.reply("🚀 开始更新 Nova Ai-Studio...\n正在拉取最新代码、重新构建...");
      try {
...

  {
    name: "status",
    aliases: ["状态"],
    description: "Show Nova Ai-Studio status and version",
    systemLevel: true,
    handler: async (ctx, _arg) => {
      const { execFile } = await import("node:child_process");
      const { resolve: resolvePath, dirname } = await import("node:path");
