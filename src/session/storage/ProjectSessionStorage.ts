...
export type AgentProjectSessionStorageOptions = {
  ...
  sessionId: string;
  now?: () => Date;
  /** PD-SAAS-FORK: authoritative jsonl when tenant scan finds session under another project dir. */
...
export function createAgentProjectSessionStorage(
  ...
  const transcriptPath = options.transcriptAbsPath
    ? resolve(options.transcriptAbsPath)
    : resolve(chatDir, `${safeId}.jsonl`);
