...
export type CanonicalMessageMetadata = {
  ...
  synthetic?: boolean;
  purpose?: string;
  /** PD-SAAS-FORK: blocks auto-continue until user acts */
  needsUserInput?: boolean;
  userActionNotice?: unknown;
  continuationOwner?: string;
  /** PD-SAAS-FORK: classify optional preference elicitation without rendering it as an error. */
  elicitationKind?: "preference" | "required" | string;
  hasDefaultOption?: boolean;
  /** PD-SAAS-FORK: expensive intent fuse (named files vs slides). */
  expensiveIntentFingerprint?: string;
  expensiveIntentHandled?: boolean;
  continuationAction?: string;
  acceptanceFailures?: string[];
  /** PD-SAAS-FORK: durable identity retained when an accepted_input is replayed. */
  acceptedInput?: {
    ref: {
      entryId: string;
      turnId: string;
...
export type ProviderConfig = {
  ...
  url: string;
  apiKey: string;
  /** PD-SAAS-FORK: backup keys tried after primary on auth/rate-limit failures. */
