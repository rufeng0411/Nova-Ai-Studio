...
export type AnthropicStreamState = {
  ...
  // PD-SAAS-FORK: strip runaway repetition from a repaired (likely-truncated) file-write content.
...
function contentBlockStopEvents(
  ...
  // PD-SAAS-FORK (flag-gated): a repaired file-write is likely truncated; if its content ran away
  // into verbatim repetition, salvage the clean prefix so the written file is usable rather than a
  // wall of repeated garbage. Conservative: only trims on detected runaway repetition.
  let finalInput = input;
  let salvaged = false;
  const toolName = toolCall.name ?? "";
  if (wasRepaired && state.salvageDegenerateWrites && FILE_WRITE_TOOL_NAMES.has(toolName)) {
    const content = readWriteToolContent(input);
    if (content !== null) {
      const salvage = salvageDegenerateContent(content);
      if (salvage.trimmed) {
        console.warn(
          `[anthropic-stream] salvaged degenerate ${toolName} (${salvage.reason}): kept `
          + `${content.length - salvage.removedChars}/${content.length} chars (index=${index})`,
        );
        finalInput = withSalvagedContent(input, salvage.content);
        salvaged = true;
      }
    }
  }
  ...
  return [
    {
      ...
      // A salvaged call is clean+complete — clear wasRepaired so the agent loop executes it (writes
      // the file) instead of blocking it as repaired-but-truncated.
      wasRepaired: salvaged ? false : wasRepaired,
      raw,
    },
  ];
}
