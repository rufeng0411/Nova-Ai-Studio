...
import {
  ...
  INVALID_MESSAGE_CONTENT_PATTERN,
...
import { isTransientNetworkErrorFromUnknown } from "../../tool/networkErrors.js";
...
export function normalizeModelError(
  ...
  const result: CanonicalModelError = {
    ...
    status,
    message,
    retryable: isRetryable(status, code, message, error),
    raw,
  };
  if (code === "prompt_too_long") {
    result.recoverableViaCompact = true;
  }
  if (MULTIMODAL_PROCESSOR_PATTERN.test(message) || INVALID_MESSAGE_CONTENT_PATTERN.test(message)) {
...
function isRetryable(
  ...
  if (message && isTransientNetworkErrorFromUnknown(message)) {
    return true;
  }
  if (error !== undefined && isTransientNetworkErrorFromUnknown(error)) {
    return true;
  }
  ...
  return ["rate_limit_error", "overloaded_error", "timeout", "server_error", "network_error"].includes(code);
}
