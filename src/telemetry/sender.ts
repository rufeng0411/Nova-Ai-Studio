...
export class TelemetrySender {
  ...
  enqueue(event: AnalyticsEvent): void {
    ...
    // PD-SAAS-FORK: redact signed URLs before telemetry can enter the durable queue.
    this.queue.push({
      event: sanitizeUrlsForPersistence(event),
      attempts: 0,
    });
  ...
  private async sendBatch(batch: AnalyticsEvent[]): Promise<void> {
    ...
    try {
      const sanitizedBatch = sanitizeUrlsForPersistence(batch);
      ...
      for (const line of lines) {
        try {
          ...
          if (parsed?.event?.eventId) {
            this.queue.push({
              event: sanitizeUrlsForPersistence(parsed.event),
  ...
  private persistQueue(): void {
    try {
      ...
      const lines = this.queue.map((item) =>
        JSON.stringify(sanitizeUrlsForPersistence(item))
      );
      writeFileSync(
