...
export type WebSubmitTurnInput = {
  ...
  projectKey?: string;
  /**
   * PD-SAAS-FORK: optional transcript storage root override. When omitted
   * (single-user / OSS), the gateway uses its construction-time pilotHome
   * (`~/.pilotdeck`) — behaviour is unchanged. In SaaS the UI passes the
   * tenant root so transcripts are stored under `<tenant>/projects/...`,
   * isolating tenants that registered the same real project path.
   */
  pilotHome?: string;
...
export type WebListSessionsInput = {
  ...
  limit?: number;
  cursor?: string;
  /** PD-SAAS-FORK: tenant transcript root override (see WebSubmitTurnInput). */
  pilotHome?: string;
};
...
export type WebReadSessionMessagesResult = {
  ...
  nextCursor?: string;
  total?: number;
  /** PD-SAAS-FORK: turnId → persisted turn_deliverable_meta from JSONL. */
  turnDeliverableMeta?: Record<string, {
    turnArtifactDir?: string;
    verifiedPaths?: string[];
    alignmentStatus?: string;
  }>;
  /** Present when only the tail of an oversized transcript was loaded. */
  transcriptWarning?: string;
  /** PD-SAAS-FORK (UDC R11): latest session deliverable manifest from JSONL. */
  sessionDeliverableManifest?: Record<string, unknown>;
  /** PD-SAAS-FORK (UDC R11): latest turn acceptance meta incl. slotBindings. */
