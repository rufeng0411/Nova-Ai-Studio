// PD-SAAS-FORK: Detect WebSocket close and notify bridge to drop stale gateway client
...
export class GatewayWsClient {
  ...
  private handleDisconnect(): void {
    ...
    this.closePending(new Error("Gateway WebSocket closed."));
    for (const handler of this.disconnectHandlers) {
      try {
        handler();
      } catch {
        // disconnect handlers must not crash the client
      }
    }
  }
