...
import { buildAcceptedInputIdentity } from "../../session/transcript/acceptedInputIdentity.js";
import type {
  SkillAddressInput,
  SkillCreateInput,
  SkillCreateResult,
...
import {
  ...
  endTurnTrace,
  getTurnTrace,
} from "../../telemetry/turnTiming.js";
import { turnStageHintText } from "../../agent/turnStageHints.js";
import { resolveTenantIdFromPilotHome } from "../../saas/tenantPaths.js";
import { resolveTrustedExecutionScope } from "../../saas/constraints/capabilityScopeContract.js";
...
export type InProcessGatewayOptions = {
  ...
  /** Optional post-turn callback. Used by createLocalGateway to
  ...
  afterTurnCompleted?: (input: {
    ...
    projectKey?: string;
    runId: string;
    /** PD-SAAS-FORK: tenant transcript root for scoped memory + summary. */
    pilotHome?: string;
    /** PD-SAAS-FORK: per-user opt-out (undefined => enabled). */
    projectContinuityUserEnabled?: boolean;
  }) => void;
  telemetry?: TelemetryClient;
  /** PD-SAAS-FORK: resolve document import context for attachment parsing. */
  resolveDocumentImportContext?: (input: {
    workspaceRoot?: string;
    tenantId?: string;
  }) => import("../../saas/document-import/types.js").DocumentImportContext | undefined;
};
...
export class InProcessGateway implements Gateway {
  ...
  async *submitTurn(input: GatewaySubmitTurnInput): AsyncIterable<GatewayEvent> {
    ...
    if (this.options.refreshConfigBeforeTurn) {
      ...
      trace.endStage("turn.config_reload", this.options.telemetry);
    } else {
      beginTurnTrace({
        runId,
        sessionId: input.sessionKey,
    ...
    const pump = (async () => {
      const trace = getTurnTrace(runId);
      const promptLanguage = input.promptLanguage;
      try {
        // PD-SAAS-FORK P0-2: tenant identity is verified from the
        // server-selected pilotHome; prompt/session ids never establish it.
        const trustedExecutionScope = resolveTrustedExecutionScope({
          pilotHomeTenantId: resolveTenantIdFromPilotHome(input.pilotHome),
          authenticatedPrincipalId:
            input.trustedExecutionScope?.principalScopeId,
          claimedScope: input.trustedExecutionScope,
        });
        trace?.beginStage("turn.session_prepare");
        queue.enqueue({
          type: "agent_status",
          event: "turn_stage",
          detail: {
            stage: "session_prepare",
            title: turnStageHintText("session_prepare", promptLanguage),
          },
        });
        const session = await this.router.getOrCreate({
          ...
          // PD-SAAS-FORK: forward the tenant transcript root so writes land
          // under the tenant home. Undefined in single-user (unchanged).
          pilotHome: input.pilotHome,
          transcriptRelPath: input.transcriptRelPath,
          projectContinuityUserEnabled: input.projectContinuityUserEnabled,
          activeRunId: runId,
          trustedExecutionScope,
        });
        trace?.endStage("turn.session_prepare", this.options.telemetry);
        ...
        trace?.beginStage("turn.attachments");
        const agentInput = await buildAgentInputWithAttachments(
          input.message,
          input.attachments,
          input.workspaceCwd ?? input.projectKey,
          this.options.resolveDocumentImportContext,
          input.pilotHome,
        );
        const acceptedInputIdentity = buildAcceptedInputIdentity(
          input.message,
          input.attachments,
        );
        trace?.endStage("turn.attachments", this.options.telemetry);
        for await (const event of session.submit(
          agentInput,
          {
            ...
            // PD-SAAS-FORK: Hub「试一下」技能锁定
            // PD-SAAS-FORK VAP/P0-I: CLI harness may send slug-only; fill displayName.
            ...(input.capabilityContext?.slug
              ? {
                  capabilityContext: {
                    ...input.capabilityContext,
                    displayName:
                      String(input.capabilityContext.displayName ?? "").trim()
                      || input.capabilityContext.slug,
                  },
                }
              : {}),
            // PD-SAAS-FORK: per-turn UI work language
            ...(input.promptLanguage ? { promptLanguage: input.promptLanguage } : {}),
            // PD-SAAS-FORK (P0-7): preserve Bridge accepted_input identity across Gateway run ids.
            ...(input.acceptedInputRef ? { acceptedInputRef: input.acceptedInputRef } : {}),
            ...(input.queueItemId ? { queueItemId: input.queueItemId } : {}),
            acceptedInputFingerprint: acceptedInputIdentity.inputFingerprint,
            acceptedInputAttachmentDescriptors: acceptedInputIdentity.attachmentDescriptors,
            trustedExecutionScope,
          },
        )) {
        ...
        endTurnTrace(runId, this.options.telemetry);
        queue.close();
      }
    })();
    ...
    try {
      ...
      resolveTurnDone();
      const sessionContext = this.router.getSessionContext(input.sessionKey);
      this.options.afterTurnCompleted?.({
        sessionKey: input.sessionKey,
        projectKey: input.projectKey,
        runId,
        pilotHome: sessionContext?.pilotHome ?? input.pilotHome,
        projectContinuityUserEnabled:
          sessionContext?.projectContinuityUserEnabled ?? input.projectContinuityUserEnabled,
      });
    }
  }
...
export function mapAgentEvent(event: AgentEvent, runId: string): GatewayEvent[] {
  switch (event.type) {
    ...
    case "turn_failed":
      // turn_completed (mapTurnCompleted) is the single UI error source to avoid duplicate banners.
      return [];
    case "session_aborted":
      return [
        {
    ...
    case "turn_acceptance_snapshot":
      return [{
        ...
        turnId: event.turnId,
        verifiedDeliverablePaths: event.verifiedDeliverablePaths,
        missingPaths: event.missingPaths,
        brokenPaths: event.brokenPaths,
        hiddenByPolicyPaths: event.hiddenByPolicyPaths,
        ...
        sessionDeliverableManifest: event.sessionManifest,
      }];
    case "session_manifest_updated":
      return [{
        ...
        turnId: event.turnId,
        sessionDeliverableManifest: event.sessionManifest,
        sessionManifestVersion: event.sessionManifest.manifestVersion,
        goalVersion: event.sessionManifest.goalVersion,
      }];
    case "turn_continued":
      return [{
        type: "agent_status",
...
function resolveGatewayErrorLabels(): UserFacingErrorLabels {
  const lang = String(process.env.PILOTDECK_PROMPT_LANGUAGE || process.env.PILOTDECK_UI_LANGUAGE || "zh-CN");
  return lang.startsWith("en") ? DEFAULT_ERROR_LABELS_EN : DEFAULT_ERROR_LABELS_ZH;
}

// PD-SAAS-FORK: mapTurnCompleted desensitizes errors + turn_completed.success
function mapTurnCompleted(result: AgentTurnResult): GatewayEvent[] {
  ...
  if (result.type === "error" && result.errors?.[0]) {
    const agentErr = result.errors[0];
    const rawForNotice = [
      agentErr.message,
      previewUnknown(agentErr.details),
    ].filter(Boolean).join("\n");
    const recoverable =
      !isInfrastructureDisconnectMessage(rawForNotice)
      && (
        isAgentErrorRecoverable(agentErr.code)
        || (
          agentErr.code === "agent_model_error"
          && isTransientModelErrorMessage(rawForNotice)
        )
      );
    const exhausted =
      agentErr.code === "agent_tool_error_loop"
      || !recoverable;
  ...
  events.push({
    ...
    receipt: result.acceptedInputReceipt,
  });
  return events;
}
...
async function buildAgentInputWithAttachments(
  ...
  workspaceRoot?: string,
  resolveDocumentImportContext?: InProcessGatewayOptions["resolveDocumentImportContext"],
  tenantId?: string,
): Promise<AgentInput> {
  const attachmentBlocks = await attachmentsToContentBlocks(
    attachments,
    workspaceRoot,
    resolveDocumentImportContext,
    tenantId,
  );
...
async function attachmentsToContentBlocks(
  ...
  const OFFICE_EXT = /\.(docx?|xlsx?|pptx?|csv|tsv)$/i;
  const importCtx = resolveDocumentImportContext?.({ workspaceRoot, tenantId });

  for (const att of attachments) {
    const resolvedPath = resolveAttachmentPath(att.path, workspaceRoot);
    ...
    const pathForResolver = resolvedPath ?? att.path;
    const lower = pathForResolver.toLowerCase();
    if (att.type === "image" || att.mimeType?.startsWith("image/")) {
      resolverRequests.push({ type: "image", path: pathForResolver, mimeType: att.mimeType });
    } else if (att.mimeType === "application/pdf" || lower.endsWith(".pdf")) {
      resolverRequests.push({ type: "pdf", path: pathForResolver });
    } else if (
      ...
      resolverRequests.push({ type: "office", path: pathForResolver });
    } else {
      resolverRequests.push({ type: "file", path: pathForResolver });
    }
  }

  if (resolverRequests.length > 0) {
    const resolved = await new AttachmentResolver({
      workspaceRoot,
      documentImport: importCtx,
      tenantId,
    }).resolveAll(resolverRequests);
...
// PD-SAAS-FORK: UI @-references use project-relative paths; join workspace root before resolve.
function resolveAttachmentPath(filePath: string | undefined, workspaceRoot?: string): string | undefined {
