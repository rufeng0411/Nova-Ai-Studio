import type { AcceptedInputCompletionReceipt, AgentTurnResult } from "../../agent/index.js";
import type {
  CronCreateInput,
  CronCreateResult,
  CronDeleteInput,
...
export type GatewaySubmitTurnInput = {
  ...
  projectKey?: string;
  /**
   * PD-SAAS-FORK: optional transcript storage root override. Omitted in
   * single-user / OSS (uses construction-time pilotHome, unchanged). SaaS
   * passes the tenant root so transcripts isolate per tenant even when two
   * tenants registered the same real project path.
   */
  pilotHome?: string;
  /** PD-SAAS-FORK: server-persisted path under pilotHome; clients cannot supply an absolute path. */
  transcriptRelPath?: string;
  /** PD-SAAS-FORK: per-user project continuity preference (false disables). */
  ...
  capabilityContext?: {
    slug: string;
    /** Optional; Gateway fills from slug when CLI harness omits it. */
    displayName?: string;
    packMemberPrefix?: string;
    majorCategory?: string;
  };
  /** PD-SAAS-FORK: UI language for work-language system prompt and recovery copy (per turn). */
  promptLanguage?: "en" | "zh-CN";
  /** PD-SAAS-FORK (P0-7): Bridge prewritten accepted_input identity; independent from runId. */
  acceptedInputRef?: AcceptedInputRef;
  /** PD-SAAS-FORK: stable queue item identity for completion acknowledgment. */
  queueItemId?: string;
  /** PD-SAAS-FORK P0-2: Bridge-authenticated scope; Gateway validates tenant against pilotHome. */
...
export type GatewayEvent =
  ...
  // PD-SAAS-FORK: turn_completed.success and error severity/hints for UI gentle notices
  | {
      type: "turn_completed";
      usage: TurnUsage;
      finishReason: AgentTurnResult["stopReason"] | string;
      success?: boolean;
      resultType?: AgentTurnResult["type"];
      receipt?: AcceptedInputCompletionReceipt;
    }
  ...
  | {
      ...
      sessionDeliverableManifest?: Record<string, unknown>;
    }
  | {
  ...
  | {
      ...
      code?: string;
      recoverable: boolean;
      severity?: "handling" | "pause" | "permission";
      hints?: string[];
      attempt?: number;
      maxAttempts?: number;
    };
...
export type WebDescribeProjectInput = {
  projectKey: string;
  /** PD-SAAS-FORK: tenant transcript root override (see GatewaySubmitTurnInput). */
  pilotHome?: string;
};
