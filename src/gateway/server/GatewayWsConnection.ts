...
import { isAcceptedInputRef } from "../../session/transcript/acceptedInputDedup.js";
  ...
  private async handleRequest(frame: WsRequestFrame): Promise<void> {
    try {
      if (frame.method === "submit_turn") {
        const submitInput = sanitizeSubmitTurnInput(frame.params);
        const sessionKey = submitInput.sessionKey;
        if (sessionKey) this.inFlightSessions.add(sessionKey);
        let seq = 0;
        let lastCompleted: GatewayEvent | undefined;
        try {
          // PD-SAAS-FORK (P0-7): preserve typed acceptedInputRef in submit_turn params.
...
function sanitizeSubmitTurnInput(value: unknown): GatewaySubmitTurnInput {
  const input = isRecord(value)
    ? { ...value } as Record<string, unknown>
    : {};
  // PD-SAAS-FORK P0-2: untrusted top-level claims are never interpreted as
  // tenant scope. The Bridge-authenticated nested envelope is still checked
  // against pilotHome by InProcessGateway.
  delete input.tenantScopeId;
  delete input.principalScopeId;
  if (
    !isRecord(input.trustedExecutionScope)
    || typeof input.trustedExecutionScope.tenantScopeId !== "string"
    || typeof input.trustedExecutionScope.principalScopeId !== "string"
    || !input.trustedExecutionScope.tenantScopeId.trim()
    || !input.trustedExecutionScope.principalScopeId.trim()
    || input.trustedExecutionScope.tenantScopeId.length > 160
    || input.trustedExecutionScope.principalScopeId.length > 160
  ) {
    delete input.trustedExecutionScope;
  } else {
    input.trustedExecutionScope = {
      tenantScopeId: input.trustedExecutionScope.tenantScopeId.trim(),
      principalScopeId: input.trustedExecutionScope.principalScopeId.trim(),
    };
  }
  if (!isAcceptedInputRef(input.acceptedInputRef)) {
    delete input.acceptedInputRef;
  }
  if (
    ...
    delete input.queueItemId;
  } else {
    input.queueItemId = input.queueItemId.trim();
  }
  return input as GatewaySubmitTurnInput;
}
