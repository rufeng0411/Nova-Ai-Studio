...
export type GatewaySessionContext = {
  ...
  channelKey: string;
  /**
   * PD-SAAS-FORK: optional transcript storage root for this session. When
   * present, the session factory stores transcripts under this root instead
   * of the gateway's construction-time pilotHome. Single-user leaves it
   * undefined (unchanged behaviour); SaaS sets the tenant root.
   */
  pilotHome?: string;
  /** PD-SAAS-FORK: stable server-persisted transcript path under pilotHome. */
  transcriptRelPath?: string;
  /** PD-SAAS-FORK: per-user opt-out for project continuity (undefined => on). */
  projectContinuityUserEnabled?: boolean;
  /** PD-SAAS-FORK: active turn id for TTFT stage correlation during session create */
  activeRunId?: string;
  /** PD-SAAS-FORK P0-2: sticky authenticated tenant/principal scope. */
...
export class SessionRouter {
  ...
  /** PD-SAAS-FORK: read sticky session context (tenant root, user prefs). */
  getSessionContext(sessionKey: string): GatewaySessionContext | undefined {
    return this.sessions.get(sessionKey)?.context;
  }
}
...
function mergeSessionContext(
  ...
  if (
    current.trustedExecutionScope
    && next.trustedExecutionScope
    && (
      current.trustedExecutionScope.tenantScopeId
        !== next.trustedExecutionScope.tenantScopeId
      || current.trustedExecutionScope.principalScopeId
        !== next.trustedExecutionScope.principalScopeId
    )
  ) {
    throw new Error(
      "trusted execution scope does not match the existing session scope",
    );
  }
  return {
    ...
    // PD-SAAS-FORK: keep the original session's tenant root sticky.
    pilotHome: current.pilotHome ?? next.pilotHome,
    transcriptRelPath: current.transcriptRelPath ?? next.transcriptRelPath,
    projectContinuityUserEnabled: current.projectContinuityUserEnabled ?? next.projectContinuityUserEnabled,
    trustedExecutionScope:
      current.trustedExecutionScope ?? next.trustedExecutionScope,
  };
}
