...
export type OrchestrationInput = {
   ...
   */
  skillPrompt?: string;
  /** PD-SAAS-FORK P0-3: skip tool stripping for deliverable-class goals. */
  preserveDeliverableTools?: boolean;
};
...
export function applyOrchestration(input: OrchestrationInput): OrchestrationResult {
  ...
  if (!input.preserveDeliverableTools && tools && config.allowedTools && config.allowedTools.length > 0) {
  ...
  } else if (!input.preserveDeliverableTools && tools && config.blockedTools && config.blockedTools.length > 0) {
    const before = tools.length;
