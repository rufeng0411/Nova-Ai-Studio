...
export const DEFAULT_SUBAGENT_MAX_TOKENS = 256_000;
...
export const DEFAULT_ALLOWED_TOOLS = [
  "agent", "read_file", "grep", "glob", "read_skill", "yixiaoer_api",
  // PD-SAAS-FORK: orchestrator may scaffold/write single-file design deliverables
  "scaffold_mobile_mockup", "write_file", "edit_file",
];
export const DEFAULT_BLOCKED_TOOLS = [
  ...
  "web_fetch",
  "fetch_page_images",
  // PD-SAAS-FORK P0-4: orchestrators delegate network localization to workers.
  "fetch_media_asset",
];
...
- \`yixiaoer_api\` — query 蚁小二 accounts or call publish/upload APIs (never search for api.ts)
...
- Spell out the exact deliverable path under the system-injected \`<task-artifact-dir path="artifacts/task-*">\` root (never legacy semantic workspace dirs).
...
If no skill template is available, use the \`<task-artifact-dir>\` path from the system prompt as the sole write root. Always pass absolute paths.`;
