...
import { shouldBypassOrchestrationForCapability, extractLatestUserGoalText } from "./orchestrate/shouldBypassOrchestration.js";
import { userGoalImpliesDeliverable } from "../agent/errors/userFacingErrors.js";
import { isAutoorchPreserveDeliverableToolsEnabled } from "../saas/resilience/stabilityFlags.js";
import type {
  RouterDecision,
  RouterDecisionInput,
...
} from "./tokenSaver/fastTokenSaverPath.js";
...
} from "../agent/loop/toolResultCompaction.js";
import {
  formatEngineLog,
  subagentBudgetExceededUserMessage,
} from "../saas/brand/userFacingProductName.js";
...
export function createRouterRuntime(
  ...
  async function decide(input: RouterDecisionInput): Promise<RouterDecision> {
    ...
    if (
      ...
      if (!stickyHit) {
        const trace = getTurnTrace(input.metadata?.runId);
        trace?.beginStage("turn.router_judge");
        const useFastPath = shouldUseNonBlockingTokenSaver({
          isMainAgent: input.isMainAgent,
          messages: input.request.messages,
          orchestrating: sticky?.orchestrating === true,
          customRouterActive: Boolean(custom?.provider),
        });
        if (useFastPath) {
          const fast = resolveFastTokenSaverDecision(config.tokenSaver, input.request.messages);
          tokenSaverTier = fast.tier;
          selection = fast.selection;
          resolvedFrom = "tokenSaver";
          void classifyAndRoute({
            ...
            .then((tokenSaver) => {
              if (!tokenSaver?.selection || !input.isMainAgent) return;
              const current = sessionStore.get(input.sessionId, false);
              if (current?.orchestrating) return;
              sessionStore.set({
                sessionId: input.sessionId,
                isSubagent: false,
                tokenSaverTier: tokenSaver.tier,
                stickyProvider: tokenSaver.selection.provider,
                stickyModel: tokenSaver.selection.model,
            ...
            .catch(() => {
              // Background judge must never affect the active turn.
            });
        } else {
        ...
        trace?.endStage("turn.router_judge", telemetry);
      }
    }
    ...
    // PD-SAAS-FORK: Pass capabilitySlug metadata; bypass orchGate for design capabilities
    const userGoal = extractLatestUserGoalText(input.request.messages);
    const bypassOrch = shouldBypassOrchestrationForCapability(
      input.metadata?.capabilitySlug,
      input.metadata?.majorCategory,
      userGoal,
    );
    const orchGate = (tokenSaverActive || alreadyOrchestrating) && !bypassOrch;
    if (bypassOrch) {
      console.log(`[router] orchestration bypassed for capability=${input.metadata?.capabilitySlug}`);
    }
    console.log(
      `[router] decision: tier=${tokenSaverTier}, model=${selection.provider}/${selection.model}, orchGate=${orchGate}, alreadyOrch=${alreadyOrchestrating}, bypassOrch=${bypassOrch}, resolvedFrom=${resolvedFrom}`,
    );
    ...
    if (config.autoOrchestrate?.enabled && orchGate) {
      const preserveDeliverableTools =
        isAutoorchPreserveDeliverableToolsEnabled()
        && Boolean(userGoal)
  ...
  async function* execute(
    ...
    const transientRetryMax = Math.max(1, config.transientRetry?.maxAttempts ?? 3);
    ...
    outer: for (let attemptIndex = 0; attemptIndex < attempts.length; attemptIndex += 1) {
      ...
      if (decision.isSubagent) {
        const budget = config.autoOrchestrate?.subagentMaxTokens ?? DEFAULT_SUBAGENT_MAX_TOKENS;
        let estimated = countMessagesTokens(attemptRequest.messages);
        if (estimated > budget) {
          const compaction = compactToolResults(
            attemptRequest.messages,
            resolveToolResultCompactionConfig(),
          );
          if (compaction.compactedBlocks > 0) {
            attemptRequest = { ...attemptRequest, messages: compaction.messages };
            estimated = countMessagesTokens(attemptRequest.messages);
          }
        }
        if (estimated > budget) {
          yield {
            type: "text_delta",
            text: subagentBudgetExceededUserMessage(estimated, budget),
