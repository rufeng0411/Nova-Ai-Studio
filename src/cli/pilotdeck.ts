...
import { loadPilotConfig, resolvePilotHome, applyYixiaoerToolEnv, applyYixiaoerToolPaths, resolvePilotDeckRepoRoot } from "../pilot/index.js";
...
import { USER_FACING_PRODUCT_NAME, formatEngineLog } from "../saas/brand/userFacingProductName.js";
...
async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  if (command === "server") {
    ...
    applyYixiaoerToolEnv(env, snapshot.config.tools?.yixiaoer, env);
    ...
    console.log(formatEngineLog(`server listening: ${server.url}`));
...
function printSkillMigrationReport(report: Awaited<ReturnType<typeof migrateSkillsToPilotDeck>>): void {
  ...
  console.log(formatEngineLog(`skills migration (${mode})`));
...
function createFallbackGateway(): Gateway {
  async function* errorStream(input: GatewaySubmitTurnInput): AsyncIterable<GatewayEvent> {
    yield {
      ...
      message: `No ${USER_FACING_PRODUCT_NAME} server is available and local config could not start session ${input.sessionKey}.`,
      recoverable: false,
    };
  }
  return {
