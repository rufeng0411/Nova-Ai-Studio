...
// PD-SAAS-FORK: missing mcp.json must not walk up to ~/.pilotdeck;
// Windows empty filenames must not invalidate all Gateway runtimes.
...
export class ExtensionWatchManager {
  ...
  private createWatchers(scope: ExtensionWatchScope, watchedPaths: string[]): FSWatcher[] {
    ...
    for (const watchedPath of watchedPaths) {
      const watchTarget = resolveWatchTarget(watchedPath);
      if (!watchTarget) {
        continue;
      }
...
/**
 * Watch an existing path, or its parent when the file has not been created yet.
 * Never walk further up — that would watch all of `~/.pilotdeck` and abort turns.
 */
export function resolveWatchTarget(watchedPath: string): string | null {
  if (existsSync(watchedPath)) {
    return watchedPath;
  }
  const parent = dirname(watchedPath);
  if (!parent || parent === watchedPath || !existsSync(parent)) {
    return null;
  }
  return parent;
}
...
/**
 * Windows `fs.watch` often emits an empty filename for directory noise.
 * Empty names are only accepted for an existing *file* watch.
 */
export function shouldHandleWatchSignal(
  watchTarget: string,
  watchedPath: string,
  filename: string,
): boolean {
  if (filename.length === 0) {
    return watchTarget === watchedPath && isExistingFile(watchTarget);
  }
