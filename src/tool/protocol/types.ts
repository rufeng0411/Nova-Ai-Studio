...
import type { GoalToolPolicy } from "../../saas/media/goalToolPolicy.js";
import type { SessionDeliverableManifest } from "../../saas/taskState/sessionDeliverableManifest.js";
import type { OfficialMediaFallbackStateMachine } from "../../saas/media/officialMediaFallbackStateMachine.js";
...
export type PilotDeckToolRuntimeContext = {
  sessionId: string;
  turnId: string;
  /** PD-SAAS-FORK: active tool call id for elicitation + UI correlation */
  toolCallId?: string;
  cwd: string;
  abortSignal?: AbortSignal;
  subagentTimeoutMs?: number;
  /** PD-SAAS-FORK (STDA): assigned task root for write guard on this turn. */
  taskArtifactDir?: string;
  /** PD-SAAS-FORK P0-4: frozen task generation bound to official media candidates. */
  taskGoalVersion?: number;
  /** PD-SAAS-FORK: session-known task roots — explicit writes stay under these paths. */
  knownTaskArtifactDirs?: string[];
  /** PD-SAAS-FORK P0-2: authenticated scope and independently recovered policy. */
  ...
  /** PD-SAAS-FORK P0-6: enforce-mode tool allowlist and task-root guard. */
  goalToolPolicy?: GoalToolPolicy;
  /** PD-SAAS-FORK ES9 P0-F: frozen SDM for sequential stage gate. */
  sessionDeliverableManifest?: SessionDeliverableManifest;
  /** PD-SAAS-FORK P0-6: shared cross-turn/subagent official-media budget. */
  officialMediaBudget?: OfficialMediaFallbackStateMachine;
  /** PD-SAAS-FORK P0-3: HTTP URLs extracted from the current trusted user input. */
