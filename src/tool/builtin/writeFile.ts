...
} from "../../saas/deliverables/bentoDeckWriteGuard.js";
import { readTextFile } from "./filesystem/readTextFile.js";
import { isParallelWriteFileAllowed } from "../../saas/taskState/parallelWritePolicy.js";

function guardedWritePath(
  filePath: string,
  context: { taskArtifactDir?: string; knownTaskArtifactDirs?: string[] },
): string {
  if (!context.taskArtifactDir) return filePath;
  return redirectWritePathToTaskDir(filePath, context.taskArtifactDir, {
...
export function createWriteFileTool(): PilotDeckToolDefinition<WriteFileInput, WriteFileOutput> {
  return {
    ...
    isReadOnly: () => false,
    isConcurrencySafe: (input) => isParallelWriteFileAllowed(input.file_path),
    isDestructive: () => true,
    validateInput: async (input, context) => {
      const guardedPath = guardedWritePath(input.file_path, context);
      const resolved = resolvePilotDeckWorkspacePath(guardedPath, context, { forWrite: true });
      if (!resolved.ok) {
        return {
          ok: false,
          issues: [{
      ...
      if (isBentoDeckWritePath(resolved.relativePath)) {
        let beforeContent = "";
        try {
          beforeContent = await readTextFile(resolved.absolutePath);
        } catch {
          beforeContent = "";
        }
        const bentoValidation = validateBentoDeckWriteContent(beforeContent, input.content);
        if (!bentoValidation.ok) {
          return {
            ok: false,
            issues: [{
              path: "content",
              code: "invalid_schema",
              message: bentoValidation.message,
            }],
          };
        }
      }

      return { ok: true, input: guardedPath === input.file_path ? input : { ...input, file_path: guardedPath } };
    },
