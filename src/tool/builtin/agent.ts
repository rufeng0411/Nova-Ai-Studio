...
import {
  promptTargetsParallelGroupOnly,
} from "../../saas/taskState/sequentialDeliverableGate.js";
import type {
...
export const BUILTIN_SUBAGENTS: Record<string, AgentSubagentDefinition> = {
  "general-purpose": {
    ...
    systemPrompt:
      "You are a general-purpose subagent inside NovaStudio. Read the user's instructions, reason carefully, and produce a single concise text answer. Do not ask follow-up questions; do your best with the information given.",
  },
  plan: {
    type: "plan",
    description:
    ...
    systemPrompt:
      "You are a planning subagent inside NovaStudio. Given a task, return a numbered plan of concrete steps a developer or operator could follow. Be specific. Do not perform the steps yourself; return the plan only.",
  },
  verify: {
    type: "verify",
    description:
    ...
    systemPrompt:
      "You are a verification subagent inside NovaStudio. Given a proposal, change, or claim, return a structured critique with: (1) specific concerns, (2) recommended checks, (3) overall verdict. Be rigorous; flag risks even if minor.",
  },
  explore: {
    type: "explore",
    description:
    ...
    systemPrompt:
      "You are an exploration subagent inside NovaStudio. Given a topic, return a structured overview: (a) common approaches, (b) trade-offs between them, (c) recommended next steps for someone unfamiliar with the area.",
  },
};
...
export function createAgentTool(
  ...
  return {
    ...
    checkPermissions: async (input, context): Promise<PermissionResult> => {
      ...
      if (
        isSequentialDeliverablesEnabled()
        && shouldEnforceSequentialDeliverables(context.sessionDeliverableManifest)
        && promptRequestsParallelDeliverableWrites(prompt)
        && !(
          isParallelOfficeExportEnabled()
        ...
        recordStabilityEvent({
          ...
          reason: "parallel_multi_file_subagent",
        });
        return {
          type: "deny",
          reason: {
            ...
            toolName: "agent",
            message:
              "Sequential deliverable mode: delegate one deliverable file at a time; parallel multi-file subagent tasks are blocked.",
          },
          message:
            "Sequential deliverable mode: delegate one deliverable file at a time; parallel multi-file subagent tasks are blocked.",
        };
...
async function runFullFork(args: {
  ...
  const taskDir = context.taskArtifactDir?.replace(/\\/g, "/").replace(/\/+$/, "");
  // PD-SAAS-FORK: rewrite legacy orchestration tmp_workspace hints to STDA task dir.
  let normalizedDirective = directive;
  if (taskDir && /\/tmp_workspace\/?/i.test(normalizedDirective)) {
    normalizedDirective = normalizedDirective.replace(/\/tmp_workspace\/?/gi, `${taskDir}/`);
  }
  const scopedDirective = taskDir
    ? [
        `任务目录（唯一写入根）：${taskDir}/`,
        "禁止写入 artifacts/ 下其它语义目录（如 nio-es9-launch/）；目录内仅用 basename。",
        isSequentialDeliverablesEnabled()
        && shouldEnforceSequentialDeliverables(context.sessionDeliverableManifest)
          ? "Sequential 模式：本次 subagent 仅允许 write_file 一个交付 basename。"
          : "",
        "",
        normalizedDirective,
      ].filter(Boolean).join("\n")
...
/** PD-SAAS-FORK ES9 P0-F: detect multi-file parallel deliverable delegation prompts. */
function promptRequestsParallelDeliverableWrites(prompt: string): boolean {
  const basenames = new Set<string>();
  for (const match of prompt.matchAll(/[\w\u4e00-\u9fff.-]+\.(?:md|html?|docx|pdf|pptx)/giu)) {
    basenames.add(match[0].toLowerCase());
    if (basenames.size >= 2) return true;
  }
  return false;
}
