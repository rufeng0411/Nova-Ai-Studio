...
import { webFetchSoftFailureText } from "../recoveryHints.js";
import type {
...
import type { PublicDnsResolver } from "./web/publicHttpUrlPolicy.js";
...
export type CreateWebFetchToolOptions = {
  ...
  temperature?: number;
  /** PD-SAAS-FORK P0-3: deterministic strict-public transport injection. */
...
export function createWebFetchTool(
  ...
  return {
    ...
    execute: async (input, context): Promise<PilotDeckToolExecutionOutput<WebFetchOutput>> => {
      ...
      try {
        // PD-SAAS-FORK P0-3: keep ordinary W7 semantics; official-media
        // contexts alone are forced through DNS-pinned publicOnly fetching.
        httpResult = await getURLMarkdownContent(url, signal, {
          publicOnly: isOfficialMediaContext(context),
          publicFetchImpl: options.publicFetchImpl,
          publicDnsResolver: options.publicDnsResolver,
        });
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        if (isTransientNetworkErrorFromUnknown(err)) {
          return {
            content: [{ type: "text", text: webFetchSoftFailureText(url, message) }],
            data: { url, fromCache: false },
      ...
      try {
        ...
        if (err instanceof PilotDeckToolRuntimeError) {
          if (err.code === "tool_aborted") throw err;
        }
        const message = err instanceof Error ? err.message : String(err);
        const fallbackText =
          `[Secondary model summarization unavailable: ${message}]\n\n` +
          "Returning raw page markdown so image CDN URLs remain usable:\n\n" +
          truncated;
        return {
