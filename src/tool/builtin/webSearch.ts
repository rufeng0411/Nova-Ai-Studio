...
import { appendEmptyWebSearchRecovery, webSearchSoftFailureText } from "../recoveryHints.js";
import type {
 ...
 */
export type WebSearchProvider = "glm" | "tavily" | "bocha" | "custom";
...
export type CreateWebSearchToolOptions = {
  ...
  /** PD-SAAS-FORK: up to 2 fallback search configs tried after primary failure. */
  fallbacks?: CreateWebSearchToolOptions[];
...
// PD-SAAS-FORK: success sentinels accepted from a `provider: custom` search proxy.
// Custom proxies disagree on the success code: serper/searxng-style proxies use
// `code: 0`, while Bocha-compatible endpoints (api.bochaai.com and HTTP-status
// mirroring proxies) return `code: 200` on success. Accepting only `0` silently
// broke every custom search pointed at a Bocha-style endpoint.
const CUSTOM_SEARCH_SUCCESS_CODES = new Set<number>([0, 200]);
...
export function createWebSearchTool(
  ...
  return {
    name: "web_search",
    aliases: ["WebSearch"],
    description: `- Searches the web using the configured provider (model-attached GLM/Z.AI web search by default)
- **Fallback order**: primary provider first; on failure automatically retries **Bocha** when \`BOCHA_API_KEY\` is set
- GEO / AI visibility tasks: prefer this tool before any third-party search APIs (do not use Perplexity/Google search keys)
...
Usage notes:
  - Configure \`tools.webSearch.provider\` as \`glm\`, \`tavily\`, \`bocha\`, or \`custom\` in \`pilotdeck.yaml\`
  - Requires \`tools.webSearch.apiKey\`, \`GLM_WEB_SEARCH_API_KEY\`/\`ZAI_API_KEY\`, \`TAVILY_API_KEY\`, \`BOCHA_API_KEY\`, or \`CUSTOM_WEB_SEARCH_API_KEY\` unless custom auth is \`none\`
  ...
  - This tool is read-only and does not modify files`,
    ...
    inputSchema: {
      type: "object",
      required: ["query"],
      // PD-SAAS-FORK: tolerate stray/hallucinated params (e.g. a weaker model
      // passing `g` instead of `gl`). Strict rejection turned a benign param typo
      // into a hard invalid_tool_input failure that burned a whole turn; ignore
      // unknown keys instead so the search still runs with the valid `query`.
    ...
    execute: async (input, context) => {
      ...
      try {
        return await executeWithCapabilityFallbacks(
          attemptConfigs.map((attemptOptions) => async () => {
            const provider = resolveProvider(attemptOptions.provider, attemptOptions.apiKey, context);
            return executeWebSearchForProvider({
              input,
              context,
        ...
        const primaryProvider = resolveProvider(options.provider, options.apiKey, context);
        const bochaKey = readEnv(context, "BOCHA_API_KEY");
        if (primaryProvider !== "bocha" && bochaKey) {
          try {
            return await executeWebSearchForProvider({
              input,
              context,
              options,
              fetchImpl,
              ...
              organicLimit,
              provider: "bocha",
              forcedApiKey: bochaKey,
            });
          } catch {
            // fall through to soft failure below
          }
        }
        if (error instanceof PilotDeckToolRuntimeError && error.code === "tool_execution_failed") {
          throw error;
        }
        const reason = error instanceof PilotDeckToolRuntimeError
          ? error.message
          : error instanceof Error
            ? error.message
            : String(error);
        return webSearchSoftFailureOutput(input.query.trim(), reason, primaryProvider);
      }
    },
  };
}

type ExecuteWebSearchForProviderInput = {
...
async function executeWebSearchForProvider(
  args: ExecuteWebSearchForProviderInput,
): Promise<PilotDeckToolExecutionOutput<WebSearchOutput>> {
  const { input, context, options, fetchImpl, timeoutMs, organicLimit, provider, forcedApiKey } = args;
  const query = input.query.trim();
  const apiKey = forcedApiKey ?? resolveApiKey(options.apiKey, provider, context);
  const custom = normalizeCustomProviderConfig(options.customProvider);
  if (!apiKey && !(provider === "custom" && custom.auth === "none")) {
    return webSearchSoftFailureOutput(
      query,
  ...
  if (provider === "bocha") {
    return performBochaSearch({
      input,
      context,
      apiKey: apiKey ?? "",
      endpoint: options.endpoint ?? DEFAULT_BOCHA_ENDPOINT,
      fetchImpl,
      timeoutMs,
      organicLimit,
    });
...
function shouldFallbackToBocha(
  result: PilotDeckToolExecutionOutput<WebSearchOutput>,
  provider: WebSearchProvider,
  bochaKey: string | undefined,
): boolean {
  if (provider === "bocha" || !bochaKey) {
    return false;
  }
  if (result.metadata?.softFailed === true) {
    return true;
  }
...
function resolveProvider(
  ...
  if (readEnv(context, "BOCHA_API_KEY")) return "bocha";
  return "glm";
}
...
async function performBochaSearch(
  args: PerformBochaSearchInput,
): Promise<PilotDeckToolExecutionOutput<WebSearchOutput>> {
  const { input, context, apiKey, endpoint, fetchImpl, timeoutMs, organicLimit } = args;
  const query = input.query.trim();
  if (!query) {
  ...
  try {
    ...
    if (controller.signal.aborted && context.abortSignal?.aborted !== true) {
      throw new PilotDeckToolRuntimeError(
        "tool_timeout",
        `web_search (bocha) timed out after ${timeoutMs}ms.`,
      );
    }
    throw new PilotDeckToolRuntimeError(
      "tool_execution_failed",
      `web_search (bocha) request failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  } finally {
    clearTimeout(timeout);
    detachAbort?.();
  ...
  if (!response.ok) {
    ...
    throw new PilotDeckToolRuntimeError(
      "tool_execution_failed",
      `Bocha API error (${response.status}): ${truncate(detail, 500)}`,
    );
  }

  const raw = (await response.json()) as Record<string, unknown>;
  const bochaCode = raw.code;
  if (typeof bochaCode === "number" && bochaCode !== 200) {
    const message = typeof raw.msg === "string" ? raw.msg : "search provider error";
    throw new PilotDeckToolRuntimeError(
      "tool_execution_failed",
      `Bocha web search error code=${bochaCode}: ${message}`,
    );
  }
  if (typeof raw.error === "string" && raw.error.length > 0) {
    throw new PilotDeckToolRuntimeError(
      "tool_execution_failed",
      `Bocha web search error: ${raw.error}`,
    );
  }

  const organic = parseBochaResults(raw, organicLimit);
  const output: WebSearchOutput = { query, organic };
  ...
  return {
    ...
    metadata: {
      provider: "bocha",
      endpoint,
      engine: "bocha-web-search",
      organicCount: organic.length,
    },
  };
}

function parseBochaResults(value: Record<string, unknown>, limit: number): WebSearchOrganicResult[] {
  ...
  return (items as Array<Record<string, unknown>>).slice(0, limit).map((entry) => ({
    title: readString(entry.name),
    link: readString(entry.url),
    snippet:
      readString(entry.summary)
      ?? readString(entry.snippet)
      ?? readString(entry.content),
    source: readString(entry.siteName),
    publishedAt:
      readString(entry.datePublished)
      ?? readString(entry.dateLastCrawled)
...
async function performCustomSearch(
  ...
  const proxyCode = raw.code;
  if (typeof proxyCode === "number" && !CUSTOM_SEARCH_SUCCESS_CODES.has(proxyCode)) {
    const message =
      typeof raw.msg === "string" && raw.msg.trim().length > 0
        ? raw.msg
...
function formatTextSummary(output: WebSearchOutput): string {
  ...
  return appendEmptyWebSearchRecovery(lines.join("\n"));
}

function webSearchSoftFailureOutput(
  query: string,
  reason: string,
  provider: WebSearchProvider,
): PilotDeckToolExecutionOutput<WebSearchOutput> {
  const normalizedQuery = query || "(empty query)";
  const output: WebSearchOutput = { query: normalizedQuery, organic: [] };
  return {
    content: [{ type: "text", text: webSearchSoftFailureText(normalizedQuery, reason) }],
    data: output,
    metadata: {
      provider,
      softFailed: true,
