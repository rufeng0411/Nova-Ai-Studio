...
import { isOfficeImportKind, detectImportKind } from "../../saas/document-import/detectImportKind.js";
...
export type CreateReadFileToolOptions = {
  resolveDocumentImportContext?: (workspaceRoot: string) => DocumentImportContext | undefined;
};
...
const FILE_UNCHANGED_STUB_PREFIX = "File unchanged:";

export function createReadFileTool(
  options: CreateReadFileToolOptions = {},
): PilotDeckToolDefinition<ReadFileInput> {
  return {
    name: "read_file",
    aliases: ["Read"],
    description:
      ...
      + "- This tool allows NovaStudio to read images (eg PNG, JPG, etc). When reading an image file the contents are presented visually when the current model supports image input\n"
    ...
    execute: async (input, context) => {
      ...
      if (previous && previous.mtimeMs === Math.floor(fileStat.mtimeMs)) {
        return {
          content: [{ type: "text", text: fileUnchangedStub(resolved.relativePath) }],
          data: {
      ...
      if (kind === "office") {
        const importCtx = options.resolveDocumentImportContext?.(context.cwd);
        if (!importCtx?.documentImport.enabled) {
          throw new PilotDeckToolRuntimeError(
            "invalid_tool_input",
            `Office file ${resolved.relativePath} must be parsed via document import (enable tools.documentImport).`,
          );
        }
        const imported = await routeImportDocument({
          sourceAbsolutePath: resolved.absolutePath,
          sourcePath: resolved.relativePath,
          workspaceRoot: importCtx.workspaceRoot,
          ctx: importCtx,
        });
        const body = imported.text
          ?? `[Could not parse ${resolved.relativePath}: ${imported.reason ?? "skipped"}]`;
        const inner = body.replace(/^<attachment[^>]*>\n?/, "").replace(/\n?<\/attachment>$/, "");
        const ranged = sliceRenderedText(inner, offset, input.limit);
        ...
        return {
          content: [{ type: "text", text: withReadFilePathHeader(resolved.relativePath, numbered) }],
          data: {
            filePath: resolved.relativePath,
            kind,
            ...
            truncated: ranged.truncated || imported.truncated,
          },
          metadata: { truncated: ranged.truncated || imported.truncated },
        };
      }
      ...
      const body = renderReadableRange(ranged.content, ranged.startLine, ranged.totalLines);
      ensureTokenBudget(body, resolved.relativePath);
      readState.set(dedupKey, {
      ...
      return {
        content: [{ type: "text", text: withReadFilePathHeader(resolved.relativePath, body) }],
        data: {
          filePath: resolved.relativePath,
          kind,
          startLine: ranged.startLine,
...
function classifyReadKind(filePath: string): "text" | "image" | "pdf" | "notebook" | "office" {
  ...
  const importKind = detectImportKind(filePath);
  if (importKind === "docx" || importKind === "xlsx" || importKind === "pptx" || isOfficeImportKind(importKind)) {
    return "office";
  }
  return "text";
}
...
function buildReadStateKey(
  filePath: string,
  kind: "text" | "image" | "pdf" | "notebook" | "office",
  offset?: number,
  limit?: number,
  pages?: string,
): string {
...
function withReadFilePathHeader(relativePath: string, body: string): string {
  // PD-SAAS-FORK: expose resolved path in model-visible text (ES9 read_file RCA).
  return `File: ${relativePath}\n${body}`;
}

function fileUnchangedStub(relativePath: string): string {
  return `${FILE_UNCHANGED_STUB_PREFIX} ${relativePath}. Refer to the earlier read_file result for this path.`;
}
