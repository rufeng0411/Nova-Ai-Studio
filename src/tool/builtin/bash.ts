...
const BASH_TOOL_DESCRIPTION = `Run a shell command in the ${USER_FACING_PRODUCT_NAME} workspace.
...
export function createBashTool(options?: CreateBashToolOptions): PilotDeckToolDefinition<BashInput> {
  ...
  return {
    ...
    execute: async (input, context) => {
      ...
      if (looksLikeRecursiveFullTreeScan(command)) {
        throw new PilotDeckToolRuntimeError(
          "invalid_tool_input",
          [
            "Recursive full-tree scan denied (e.g. dir /s /b on a large folder).",
            "Use grep or glob scoped to artifacts/ or a known subfolder instead.",
            "",
            "Recovery: read known paths under artifacts/ (e.g. @artifacts/campaign/…);",
            "do not scan the entire workspace with shell find/dir.",
          ].join("\n"),
        );
      }
...
function looksLikeRecursiveFullTreeScan(command: string): boolean {
  return /\bdir\s+\/s\s+\/b\b/i.test(command)
    || /\bfind(?:\.exe)?\s+[^\s|;&]+(?:\s+-[^\s|;&]+)*\s+-name\b/i.test(command)
    || /\bGet-ChildItem\b[^|;&]*-Recurse\b/i.test(command);
}
