...
import { type ChildProcess, spawn } from '../../util/childProcess.js';
...
export class BackgroundTaskRuntime {
  ...
  async start(spec: StartTaskSpec): Promise<PilotDeckBackgroundBashTask> {
    ...
    try {
      const resolved = resolveHiddenShellSpawn(spec.command, {
        cwd: spec.cwd,
        env: spec.env,
      ...
      child =
        resolved.args.length > 0
          ? this.options.spawn(resolved.command, resolved.args, withHiddenConsole(resolved.options))
          : this.options.spawn(resolved.command, withHiddenConsole(resolved.options));
