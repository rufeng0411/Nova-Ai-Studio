...
// PD-SAAS-FORK: work-language + gentle-recovery core strategy injection
import type { PromptLanguage } from "./resolvePromptLanguage.js";
import { buildSaasCoreStrategy } from "./saasCoreStrategy.js";
...
export type PromptAssemblerInput = {
  ...
  now?: () => Date;
  // PD-SAAS-FORK: UI/system language driving the work-language core strategy.
  promptLanguage?: PromptLanguage;
  /** PD-SAAS-FORK: session goal for creative prefer-generate_image degrade block. */
  sessionUserGoal?: string;
...
export class PromptAssembler {
  ...
  assemble(input: PromptAssemblerInput): PromptAssemblerResult {
    ...
    // PD-SAAS-FORK: always inject the core strategy (work language, delivery
    // self-check, gentle recovery) — also when a custom system prompt is set.
    parts.push(buildSaasCoreStrategy(input.promptLanguage ?? "en", {
      userGoal: input.sessionUserGoal,
      capabilitySlug: input.capabilitySlug,
    }));
    if (input.promptLanguage === "zh-CN") {
      parts.push(
        "<thinking-language>用简体中文进行扩展思考、推理与过程叙述；代码、路径、API 名与英文教学例句除外。</thinking-language>",
      );
    }
  ...
  private buildDefaultSystemPrompt(input: PromptAssemblerInput): string[] {
    const lines: string[] = [
      `You are ${USER_FACING_PRODUCT_NAME}, an AI agent runtime. You execute tasks across CLI, TUI, web, and chat channels by calling structured tools and reasoning over their results.`,
  ...
  switch (mode) {
    ...
    case "plan":
      return [
        ...
        "- DO NOT call write_file, edit_file, create_file, or bash with any command that writes, deletes, or modifies files, except for markdown plan files under `.pilotdeck/plans/`",
