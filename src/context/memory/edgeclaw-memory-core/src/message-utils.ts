...
// PD-SAAS-FORK: match injected tool-recovery user bubbles (no metadata on wire).
const TOOL_RECOVERY_PREFIX = "Several tools failed in a row.";

function isToolRecoveryUserText(content: string): boolean {
  const trimmed = content.trim();
  return trimmed.startsWith(TOOL_RECOVERY_PREFIX) || trimmed.includes("Failed tools:");
}
...
function shouldSkipUserMessage(content: string): boolean {
  ...
  return isSessionStartupMarkerText(content)
    ...
    || isToolRecoveryUserText(content);
}
...
function normalizeSingleMessage(
  ...
  const rawContent = msg.content ?? nestedMessage?.content;
  const rawUserText = role === "user" ? extractTextFromContent(rawContent).trim() : "";
  const content = info.content;

  if (role === "user" && (shouldSkipUserMessage(content) || isToolRecoveryUserText(rawUserText))) return undefined;
...
export function normalizeMessages(
  ...
  // PD-SAAS-FORK: last_turn anchor fallback when the trailing user was skipped
  // (e.g. tool-recovery bubble) — walk raw messages for the latest valid user.
  for (let i = rawMessages.length - 1; i >= 0; i -= 1) {
    const normalized = normalizeSingleMessage(rawMessages[i], options);
    if (normalized?.role !== "user") continue;
    const tail: MemoryMessage[] = [normalized];
    for (let j = i + 1; j < rawMessages.length; j += 1) {
      const followUp = normalizeSingleMessage(rawMessages[j], options);
      if (followUp) tail.push(followUp);
    }
    return tail;
  }
