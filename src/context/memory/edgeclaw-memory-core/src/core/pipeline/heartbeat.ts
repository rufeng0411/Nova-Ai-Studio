...
/** PD-SAAS-FORK: transcript-backed context for turn_complete project summary. */
  ...
  async runHeartbeat(options: HeartbeatRunOptions = {}): Promise<HeartbeatStats> {
    ...
    for (const sessionKey of sessionKeys) {
      ...
      if (
        ...
        try {
          let summaryPromptDebug: RetrievalPromptDebug | undefined;
          ...
          createStep(
            trace,
            "project_turn_summary",
            "Project Turn Summary",
            summaryCandidate ? "success" : "skipped",
            previewText(ctx.anchorUserText || ctx.assistantDeliveryText, 180),
            summaryCandidate ? `created=${summaryCandidate.name}` : "skipped=turn_summary",
            {
              ...(summaryPromptDebug ? { promptDebug: summaryPromptDebug } : {}),
            },
          );
          if (summaryCandidate) {
            const focusTurn: MemoryMessage = {
              role: "user",
