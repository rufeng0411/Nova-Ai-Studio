...
`.trim();
// PD-SAAS-FORK: turn-complete project continuity summary (user anchor + assistant delivery).
const PROJECT_TURN_SUMMARY_SYSTEM_PROMPT = `
You create one append-only project memory note summarizing a completed conversation turn.

Rules:
- Output exactly one project note for continuity across later turns in the same project.
- Include: task intent, key conclusions, deliverable file paths (if any), and actionable next-step context.
- Use the anchor user message language (Chinese if the user wrote Chinese).
- Do NOT store tool recovery errors, permission prompts, bash failures, or transient debugging.
...
function recallProjectSourcePriority(project) {
    if (project.sourceType === "general_local" || project.sourceType === "workspace_external_mirror")
        return 2;
    if (project.sourceType === "workspace_external")
        return 1;
    return 0;
}
function chooseBestRecallProjectFallback(shortlist) {
...
export class LlmMemoryExtractor {
    ...
    /** PD-SAAS-FORK: project turn summary after user turn completes (continuity). */
    async createProjectTurnSummary(input) {
        const requestLabel = "Project turn summary";
        const systemPrompt = PROJECT_TURN_SUMMARY_SYSTEM_PROMPT;
        const userPrompt = JSON.stringify({
            anchor_user_message: input.anchorUserText,
            assistant_delivery: input.assistantDeliveryText,
            deliverable_paths: input.deliverablePaths,
            current_project_meta: input.currentProjectMeta
                ? {
                    projectName: input.currentProjectMeta.projectName,
                    description: input.currentProjectMeta.description,
                    status: input.currentProjectMeta.status,
                }
                : null,
        }, null, 2);
        ...
        try {
            ...
            this.logger?.warn?.(`[clawxmemory] project turn summary fallback: ${String(error)}`);
            return null;
        }
    }
