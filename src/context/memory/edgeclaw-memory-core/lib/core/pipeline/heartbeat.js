...
function tokenizeSearchText(value) {
    ...
    return Array.from(new Set(value
        ...
        .filter((item) => item.length >= 2))).filter((item) => !stopwords.has(item));
}
function buildGeneralProjectShortlist(catalog, text) {
...
function buildCandidateRoutingQuery(candidate, focusTurn) {
    return [
        focusTurn.content,
        candidate.name,
...
function buildCandidateMemoryPreview(candidate) {
    const lines = [
        `# ${candidate.name}`,
        "",
        `type: ${candidate.type}`,
    ...
    if (candidate.type === "feedback") {
        ...
        if (candidate.howToApply)
            lines.push("", "## How To Apply", candidate.howToApply);
    }
    if (candidate.type === "project") {
        if (candidate.stage)
            lines.push("", "## Current Stage", candidate.stage);
        const sections = [
...
export class HeartbeatIndexer {
    ...
    async routeGeneralCandidate(input) {
        ...
        let selectedProject;
        let selectionDebug;
        let routingDecision = "create_new";
    ...
    async runHeartbeat(options = {}) {
        ...
        for (const sessionKey of sessionKeys) {
            ...
            for (const session of sessions) {
                try {
                    ...
                    for (const focusTurn of focusUserTurns) {
                        ...
                        let labels = classification.shouldStore ? classification.labels : [];
                        // PD-SAAS-FORK: turn_complete uses dedicated PROJECT_TURN_SUMMARY for project.
                        if (options.reason === "turn_complete") {
                            labels = labels.filter((label) => label.type !== "project");
                        }
                        ...
                        for (const { candidate } of createdCandidates) {
                            ...
                            if (routed.createdProjectMeta) {
                                ...
                                createStep(trace, "project_routed", "Project Routed", "success", previewText(focusTurn.content, 180), `Created new General project ${routed.createdProjectMeta.projectName}.`, {
                                    details: [
                                ...
                                createStep(trace, "project_routed", "Project Routed", "success", previewText(focusTurn.content, 180), `Assigned to ${routed.selectedProject.projectName}.`, {
                                    details: [
                            ...
                            if (routed.candidate.type === "project")
                                stats.writtenProjectFiles += 1;
                            if (routed.candidate.type === "feedback")
            ...
            // PD-SAAS-FORK: turn_complete project continuity summary (transcript-backed).
            if (options.reason === "turn_complete"
                && options.turnSummaryContext
                && !options.turnSummaryContext.optOut) {
                const ctx = options.turnSummaryContext;
                const summaryTimestamp = sessions[sessions.length - 1]?.timestamp ?? nowIso();
                try {
                    let summaryPromptDebug;
                    const summaryCandidate = await this.extractor.createProjectTurnSummary({
                        timestamp: summaryTimestamp,
                        sessionKey,
                        anchorUserText: ctx.anchorUserText,
                        assistantDeliveryText: ctx.assistantDeliveryText,
                        deliverablePaths: ctx.deliverablePaths,
                        currentProjectMeta: this.repository.getProjectMeta() ?? store.getProjectMeta() ?? null,
                        debugTrace: (debug) => {
                            summaryPromptDebug = debug;
                        },
                    });
                    createStep(trace, "project_turn_summary", "Project Turn Summary", summaryCandidate ? "success" : "skipped", previewText(ctx.anchorUserText || ctx.assistantDeliveryText, 180), summaryCandidate ? `created=${summaryCandidate.name}` : "skipped=turn_summary", {
                        ...(summaryPromptDebug ? { promptDebug: summaryPromptDebug } : {}),
                    });
                    if (summaryCandidate) {
                        const focusTurn = {
                            role: "user",
                            content: ctx.anchorUserText || ctx.assistantDeliveryText || "turn summary",
                        };
                        const routed = await this.routeGeneralCandidate({
                            candidate: summaryCandidate,
                            focusTurn,
                            batchContextMessages: flattenBatchMessages(sessions, previousMessages),
                        });
                    ...
                    sessionHadError = true;
                    this.logger?.warn?.(`[clawxmemory] project turn summary failed for ${sessionKey}: ${String(error)}`);
                }
            }
