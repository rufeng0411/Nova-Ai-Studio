...
function parseRawRecallSections(value) {
    const sections = {
        userProfile: [],
        projectMeta: [],
        projectMemory: [],
    ...
    for (const rawLine of value.split(/\r?\n/u)) {
        ...
        if (line === "## ClawXMemory Recall")
            continue;
        if (/^route=/u.test(line))
            continue;
...
function normalizeCaseTraceContextPreview(record) {
    const retrieval = record.retrieval;
    const preview = retrieval?.contextPreview;
    if (!preview || !retrieval || !looksLikeRawRecallContext(preview))
        return record;
    return {
        ...record,
        retrieval: {
...
export class EdgeClawMemoryService {
    ...
    async flush(options = {}) {
        ...
        const stats = await this.indexer.runHeartbeat({
            batchSize: options.batchSize,
            sessionKeys: options.sessionKeys,
            reason: options.reason,
            turnSummaryContext: options.turnSummaryContext,
        });
