...
export class EdgeClawMemoryProvider implements MemoryResolver {
  ...
  async captureTurn(input: MemoryCaptureTurnInput): Promise<void> {
    // PD-SAAS-FORK: drop synthetic recovery/continuation user bubbles before
    // metadata is stripped by canonicalMessagesToMemoryMessages.
    const filteredMessages = input.messages.filter((message) => {
      if (message.metadata?.synthetic === true) return false;
      return true;
    });
    const normalizedMessages = canonicalMessagesToMemoryMessages(filteredMessages);
    this.options.telemetry?.trackFeatureLoopStage({
      module: "memory",
      ownerModule: "memory",
      executionKind: "memory",
    ...
    try {
      ...
      // PD-SAAS-FORK: surface capture failures to the gateway stderr log.
      // Previously this catch was fully silent (telemetry is off by default),
      // so the "database is not open" lifecycle bug that emptied SaaS memory
      // left no trace and was very slow to diagnose. We still swallow the
      // error so a failed capture never breaks the agent turn, but a one-line
      // warning makes regressions observable without manual instrumentation.
      // eslint-disable-next-line no-console
      console.warn(
        `[memory] captureTurn failed (session=${input.sessionId}): ${
