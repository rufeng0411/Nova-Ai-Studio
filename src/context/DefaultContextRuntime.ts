...
// PD-SAAS-FORK: prompt language type for core-strategy injection
import type { PromptLanguage } from "./prompt/resolvePromptLanguage.js";
...
export type AutoCompactResult =
  ...
  // PD-SAAS-FORK: tier-3 summarization deferred to post-turn background job
  | {
      type: "deferred";
      messages: CanonicalMessage[];
      tier: "full";
...
export type DefaultContextRuntimeOptions = {
  ...
  now?: () => Date;
  // PD-SAAS-FORK: UI/system language forwarded to PromptAssembler core strategy.
...
/** PD-SAAS-FORK: skip memory retrieval on the first real assistant turn (P2-2 TTFT). */
export function shouldSkipMemoryRetrievalForTurn(messages: CanonicalMessage[]): boolean {
  return !messages.some((message) => message.role === "assistant" && !message.metadata?.synthetic);
}
...
export class DefaultContextRuntime implements ContextRuntime {
  ...
  // PD-SAAS-FORK
  private readonly promptLanguage?: PromptLanguage;
  ...
  constructor(options: DefaultContextRuntimeOptions = {}) {
    ...
    this.promptLanguage = options.promptLanguage;
  }
  ...
  async prepareForModel(input: ContextPrepareInput): Promise<ModelContext> {
    ...
    const prompt = this.promptAssembler.assemble({
      ...
      now: this.now,
      promptLanguage: input.promptLanguage ?? this.promptLanguage,
      // PD-SAAS-FORK: creative prefer-generate_image degrade needs goal
      sessionUserGoal: input.sessionUserGoal,
      capabilitySlug: input.capabilitySlug,
    });

    const parts = [...prompt.parts];
    if (this.memoryAttachmentBuilder && !shouldSkipMemoryRetrievalForTurn(projection.messages)) {
      const trace = getTurnTrace(input.turnId);
      trace?.beginStage("turn.memory_retrieve");
      ...
      trace?.endStage("turn.memory_retrieve");
    ...
    return {
      ...
      cacheBreakpoints:
        microcompactResult?.cacheBreakpoints && microcompactResult.cacheBreakpoints.length > 0
          ? microcompactResult.cacheBreakpoints
          : joined.length > 0
            ? [0]
            : undefined,
    };
  ...
  async tryAutoCompact(input: {
    ...
    maxContextTokens?: number;
    /** When false, tier-3 model summarization is deferred (non-blocking). */
    allowFullCompaction?: boolean;
    ...
    if (this.compactionEngine) {
      if (input.allowFullCompaction === false) {
        return {
          type: "deferred",
          messages,
          tier: "full",
...
function extractRecentUserText(messages: CanonicalMessage[]): string | undefined {
  ...
  // PD-SAAS-FORK (Goal Loop R9 C1): anchor memory retrieve to session deliverable goal, not「继续」追问。
  if (isContinuationOnlyUserText(latestUser)) {
    const sessionGoal = extractDeliverableSessionUserGoal(messages);
    if (sessionGoal.trim()) return sessionGoal;
  }
  return latestUser;
}
