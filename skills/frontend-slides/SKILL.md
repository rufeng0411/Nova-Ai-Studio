...
Write the presentation under a **unique task folder** so repeat runs in the same project do not overwrite prior decks:

- Path: `artifacts/slides-<topic-slug>-<YYYYMMDD-HHmm>/index.html` (or `slides.html` if multi-file)
- `<topic-slug>`: short kebab-case from the deck topic; append local date-time for uniqueness
- In chat, cite the **full relative path** — never bare `index.html` alone
- See `skills/open-design/references/artifact-naming.md` for the same rules used by design pages
