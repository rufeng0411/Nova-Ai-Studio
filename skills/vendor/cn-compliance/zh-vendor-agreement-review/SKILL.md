---
name: zh-vendor-agreement-review
description: >
...
**在阅读合同之前，阅读 `(本机 Claude 插件路径已移除)`。** 如果文件缺失或仍有占位符，弹出以下提示：
...
`(本机 Claude 插件路径已移除)` 中的审查指引是真实来源。它告诉你本团队的标准立场、曾经接受的让步、从不接受的内容、审批权限以及需要首先检查的deal-breaker。
