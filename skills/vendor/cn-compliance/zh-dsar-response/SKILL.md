...
1. 加载 `(本机 Claude 插件路径已移除)` → DSAR 流程（系统清单、验证方式、SLA）。
2. 执行以下工作流。
...
读取 `(本机 Claude 插件路径已移除)` → `## DSAR 流程`。该节包含：
- 系统清单（用户数据所在的每个位置）
- 身份验证方式
- 回复 SLA
- 谁处理常规请求 vs. 谁处理升级请求
...
按 `(本机 Claude 插件路径已移除)` 中的验证方式。常见方式：
...
遍历 `(本机 Claude 插件路径已移除)` 中的系统清单。对每个系统：
...
**在将任一函件发送给个人信息主体之前：** 读取 `(本机 Claude 插件路径已移除)` 中的 `## 谁在使用`。如果角色为非律师：
...
> **注意：** 两份 DSAR 函件均为对外发送给个人信息主体的文件。**勿**在两份函件上包含 `(本机 Claude 插件路径已移除)` `## 输出` 中的工作成果抬头。函件附随的内部笔记、日志和豁免分析是律师工作成果——将这些分开保存并冠以工作成果抬头。
...
按 `(本机 Claude 插件路径已移除)` → 升级表，以下情况升级：
...
如果 `(本机 Claude 插件路径已移除)` → `## DSAR 流程` 记录了一个比法定期限更严格的内部 SLA，使用内部 SLA 并注明法定底线。
