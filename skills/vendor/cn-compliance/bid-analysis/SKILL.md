...
**唯一允许的写入方式：bash heredoc 分段写入 `分析报告.md`——第一段 `cat >` 创建，后续全部 `cat >>` 追加，分 5-8 段写完。**

**以下行为全部禁止，违反任何一条即任务失败：**
1. 禁止用 write 工具写 `分析报告.md`（覆盖模式，丢失前文）
2. 禁止对 `分析报告.md` 第二次使用 `cat >`（同样覆盖）
3. 禁止创建任何临时/中间 md 文件再合并（只允许存在 `分析报告.md` 一个 md 输出）
...
3. **信息合并原则**：
   - 主文件与Excel/其他附件对同一项（如评分标准）描述不一致时，**不预设"主文件为准"**——按下文"冲突与不一致的处理原则"执行：先查找招标文件是否自带裁决规则（如"以XX为准"的效力顺序条款），找到则按规则裁决；找不到规则才标注存疑并质询
   - Excel 的技术参数补充到技术需求章节
   - 合同的付款条件补充到商务要求章节
   - 标注每条信息的来源文件

**多文件合并完整示例**：多文件场景下，写报告前必须先读同目录的 `MULTI_FILE_EXAMPLE.md`——它演示了主文件+Excel+合同模板如何合并为一份带来源标注的分析报告。
...
### 1.6 招标文件性质校验（必须执行，不可跳过）

**⚠️ 在进入深度分析之前，必须确认上传的文件确实是招标/采购文件。** 如果文件不是招标文件却继续分析，下游所有 skill 的输出都将是无意义的幻觉。

#### 1.6.1 招标文件识别特征

从已读取的文档内容中，检查以下特征（**至少命中 3 项**才通过校验）：

| 类别 | 必备特征 | 判断方式 |
|------|---------|---------|
| 标题特征 | 文件名或文档标题含"招标""采购""磋商""谈判""询价""投标" | 关键词匹配 |
| 编号特征 | 文档含采购编号/项目编号/招标编号（格式如 XX-2024-XXX） | 正则匹配编号模式 |
| 主体特征 | 文档含明确的采购人/招标人/采购单位名称及联系方式 | 查找"采购人""招标人"字样 |
| 程序特征 | 文档含"投标人须知""供应商须知""采购须知"章节 | 章节标题匹配 |
| 评审特征 | 文档含评审办法/评分标准/评审因素/评标委员会 | 关键词匹配 |
| 资格特征 | 文档含资格要求/资格条件/资格审查/特定资格 | 关键词匹配 |
| 技术特征 | 文档含技术规范/技术需求/技术规格/功能需求 | 关键词匹配 |
| 格式特征 | 文档含投标文件格式/响应文件组成/附件清单 | 关键词匹配 |
| 法规引用 | 文档引用招标投标法/政府采购法/财政部令第87号等 | 法规名称匹配 |
| 合同特征 | 文档含采购合同模板/合同条款（与采购标的直接相关） | 合同内容与采购上下文一致 |

**类别说明**（基于国内招标法规体系）：

采购项目根据是否与"工程建设"相关，适用两套不同的法律体系：

| 项目类型 | 典型内容 | 适用法规 |
|---------|---------|---------|
| 与工程建设相关的货物/服务 | 构成工程不可分割的组成部分，且为实现工程基本功能所必需（如教学楼门窗、设计监理） | 《招标投标法》及其实施条例 |
| 与工程建设无关的货物/服务 | 日常办公用品、物业管理、课题研究、信息化系统 | 《政府采购法》及其实施条例；如采用招标方式，还需遵守财政部令第87号 |

两类项目均在本 skill 的识别范围内，但**文档必须具有招标/采购性质**。

#### 1.6.2 校验流程

**步骤一：特征扫描**

从已读取的文档全文（标题、目录、前 50 页内容）中扫描上述 10 类特征的命中情况。
...
|-----------|------|------|
| ≥ 5 | ✅ 确认招标文件 | 正常进入步骤 2 |
| 3-4 | ⚠️ 疑似招标文件 | 向用户确认："检测到文档含{具体命中特征}，疑似招标文件但特征不够充分。请确认这是否为正式的招标/采购文件？" |
| 1-2 | ⚠️ 高度存疑 | 向用户确认并列出缺失特征："文档仅含{命中特征}，缺少招标文件的核心标识（如采购编号、评审标准、资格要求等）。请确认该文档的性质。" |
| 0 | ❌ 非招标文件 | **果断拒绝**："该文档未检测到任何招标/采购文件的识别特征，不属于招标/采购文件。本 skill 仅处理政府采购招标/磋商文件，无法对该文档进行分析。" |

**步骤三：明确拒绝场景**

以下类型的文件**必须拒绝**，即使命中 1-2 项特征：
- 纯企业内部管理文件（会议纪要、内部报告、工作日志）
- 学术论文、研究报告、政策解读（不含具体采购内容）
- 产品宣传册、技术白皮书（无采购主体）
- 法律法规条文汇编（无具体采购项目）
- 纯合同文件（无招标前置流程）
...
**AUTO_MODE 行为**：
- ≥ 5 命中 → 自动通过
- 3-4 命中 → 标注风险但继续（在分析报告中注明"⚠️ 招标文件特征命中仅 N 项，可能存在误判"）
- 0-2 命中 → **直接 FAILED**，不进行后续分析

#### 1.6.3 校验结果记录

在分析报告的"项目概况"节之前，插入校验结果：

```markdown
> 📋 招标文件校验: ✅ 通过（命中 N/10 项特征）
> 命中特征: {具体命中的特征类别列表}
```
...
- **Excel 数据处理**：
  ...
  - 评分标准表：如Excel与主文件均有评分表且内容不一致，**不预设"以主文件为准"**——按下文"冲突与不一致的处理原则"处理，先查找招标文件是否自带裁决规则，找不到才存疑质询；内容一致或Excel只是补充细化时，正常合并即可
...
### ★实质性条款（全文闭环核对，强制）：招标文件的★不只出现在技术要求表，商务要求表、供应商须知附表均可能各自标★，任一★未响应或不满足=投标无效。提取流程：① grep 星号字符扫所有正文章节（含第六章商务要求、第六章技术要求），逐一记录所在表格行号+章节名；② 按 商务★/技术★/其他★ 三类分别列出，每类给序号、原文条款摘要、精确来源；③ 与符合性审查表中 星号条款 类目交叉对照——符合性审查表提到 N 类★，本节必须对应不少于 N 个独立★清单。输出表格列：类别|序号|条款摘要(原文)|精确来源。
...
**关键注意事项：**
- **术语只认原文，不预设对应关系**：封面用"投标文件"还是"报价文件"、"投标人"还是"报价供应商"，完全以招标文件原文实际写法为准，逐字照抄。**不要根据"这是军队采购/政府采购项目"去推断或纠正术语**——同一采购类型下不同项目的措辞习惯并不统一，凭采购类型预判术语本身就是一种"按经验推测"，属于本 SKILL.md"数据准确性"章节明确禁止的行为。
...
3. **不对应任何评分项的文件（资格证明、格式文件、声明函等），且从文件名/用途上看明显是行政性/资质性文件（如营业执照、承诺函、保密协议）→ 标注"商务标"**

4. **不对应任何评分项、且内容实质是技术性质的文件（如技术方案编制说明、技术团队介绍等文件名/用途明显偏技术）→ 不可直接套用规则3默认成"商务标"**。此时先回招标文件核实：
   - 检查该文件是否在"投标文件组成"章节被明确归入某一册/某一类（分册结构可能已给出答案，见规则1）
   - 检查该文件所述内容是否与技术需求章节的某项要求对应（即使评分标准未单独设分值，招标文件正文可能仍说明了该文件应归入技术部分）
   - 找到依据 → 按依据标注；**确实找不到任何依据** → 标注归属的同时在该行"精确来源"列注明"⚠️归属存疑，招标文件未明确规定，按内容性质暂定为{商务标/技术标}，建议核实"，供 bid-verification 阶段复核，不可当作确定结论悄悄归入"商务标"了事
...
|------|------------|---------|----------|------|
| **封面格式** | 逐字按原文格式制作，术语（投标文件/报价文件、投标人/报价供应商等）以原文实际写法为准，不预设与采购类型的对应关系 | （见本报告"封面格式"节，已逐行逐字提取） | 报价文件内容及格式章节（封面模板） | 字段名错误是低级废标风险，下游skill必须原文照搬 |
...
- **封面格式**：必须逐字提取原文字段名，不可套用经验模板。术语（投标文件/报价文件、投标人/报价供应商等）以原文实际写法为准，不得凭"这是军队/政府采购项目"去推断或纠正
...
- **格式文件与封面格式是否已提取**（对照以下清单）：
  ...
  - [ ] 封面字段名（含"投标/报价""投标人/报价供应商"等术语）已逐字与原文核对，未凭采购类型推断或替换
...
- **写入方式**：严格按照 `references/file-writing.md` 中的文件生成规范，使用 `bash cat >` 创建 + `cat >>` 追加，禁止使用 write 工具
- **路径**：工作目录下（相对路径）
...
| ★实质性条款(商务) | grep 第六章商务要求表带★行数 |
| ★实质性条款(技术) | grep 第六章技术要求表带★行数 |
| ★总数闭环 | 商务★+技术★+其他★ 须等于符合性审查表对应类目数 |
| 格式文件夹ls计数 | ls 统计格式文件夹内 docx+pdf+xlsx 文件数 |
| 格式文件清单表行数 | 统计"格式文件清单"表的行数；与"格式文件夹ls计数"必须相等 |

**执行顺序**：先完成分析报告全文 → 再对上表逐项计数 → 最后填入下方摘要模板。不可在写报告过程中"顺手记一个大概数字"就直接用于摘要。

**🚨 grep 计数不是权威来源，只能做交叉校验**：`grep -c` 数的是"行"，不是"表格行"——原表格如有合并单元格（rowspan）、单元格内换行、或某行内容跨多行渲染，grep 数出的行数会和真实表格行数不一致（真实案例：技术评审项数 37→35，用 grep 数星号/关键字数出的"37"和实际逐行读表数出的"35"不一致，且当时自检环节也误用了错误的"37"去核对，说明生成和自检用了不一致的计数口径）。**权威计数方法按以下优先级：**
1. **最权威**：对原始 docx 直接跑 `len(doc.tables[N].rows)`（python-docx），这是表格结构层面的真实行数，不受 Markdown 渲染或措辞影响
2. **次优**：人工逐行读表格数（打开原表格，一行一行数，不跳读、不估算）
3. **仅用于交叉校验，不可单独作为最终依据**：`grep -c` 关键字/符号出现次数——如与方法1/2结果不一致，**以方法1/2为准**并回查 grep 模式为何漏行/多计（常见原因：单元格内换行导致一行内容被 grep 拆成多行，或关键字在同一行内出现多次被重复计数）

生成阶段的计数和自检阶段的复核**必须使用同一权威方法**重新各算一次，不可让自检直接照抄生成阶段的数字"过一眼觉得像就通过"。
