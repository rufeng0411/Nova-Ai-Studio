...
**唯一允许的写入方式：bash heredoc 分段写入——第一段 `cat >` 创建，后续全部 `cat >>` 追加；修复模式用 read + edit 局部替换。**

**以下行为全部禁止，违反任何一条即任务失败：**
1. 禁止用 write 工具多次写同一文件（覆盖模式，丢失前文）；禁止 read+write 假追加
2. 禁止写 Python/Node.js 脚本批量生成响应内容（会退化为统一模板，重复率 95%）——必须逐条读需求原文、逐条编写
3. 禁止在 Markdown 中使用 `&nbsp;` 等 HTML 实体（转 Word 后显示为纯文本，破坏排版）

**写文件前必须先读本 skill 目录下的 `references/file-writing.md`，按其规范执行。**
...
- **`system_decomposition.json`**（如存在，工作目录根）— 系统分解单一事实源（由 bid-system-decomp 生成）。**技术标中一切系统编号（S01/S02…）、系统名称、功能截图占位符均以此为准**，不得自行编号或命名
...
**特殊情况（所有档位通用）：**
   - **▲标注项（软件功能类）**：响应说明后额外加一行截图占位符 `【此处插入:截图:<FP-id>】`（截图需加盖公章），其中 `<FP-id>` 取该条目对应功能点在 `system_decomposition.json` 的 id（占位符格式与登记规则详见 4.2.1）；硬件/物理参数类 ▲ 条目不插截图占位符
...
3. **▲标注完整性检查**：
   ...
   echo "截图占位符数: $(grep -c '此处插入:截图:' 响应文件/*.md)"
   ```
   - 软件功能类 ▲ 标注数应与截图占位符数对应
...
4. 为每个子维度编写实质内容
   ...
   **🆕 与兄弟文件的分层原则（强制）：** 同一评分子维度在偏离表（是否响应）/ 证明材料（用什么证明）/ 技术方案（怎么实现）三份文件中应呈现不同侧面，避免重复：
   - 技术方案章节**必须讲实现机制**（怎么做、用什么技术栈、如何组织、如何保障）——禁止只列规格表不讲实现
   - **禁止复制兄弟文件**（偏离表 / 证明材料）的规格表或"完全响应"类响应文字作为技术方案主体
   - **规格密集型章节**（如资源量清单、设备清单、人员清单）在技术方案中只做一两行总结，并链接到偏离表/证明材料（如"详见偏离表第N条""完整清单见证明材料附件X"），不再抄表
...
- 示例：
  ...
  **对接机制：**
  ...
  【此处插入:图表:ops-three-tier-integration】
...
**占位符 id（slug）取法：** `<slug>` = 该图的**语义短标识**，由编写者自拟，需稳定、有意义、且**同一项目内唯一**（不重复）。用小写英文 + 短横线，简短达意即可——下游 `bid-mermaid-diagrams` 按此 slug 在 `placeholders.json` 精确定位、在正文按 id 替换，slug 就是唯一连接键：
- ✅ `sys-arch`（系统总体架构图）、`role-perm`（角色权限图）、`exam-flow`（在线考试流程）、`ops-org-arch`（运维组织架构图）、`data-pipeline`（数据处理流水线）
- ❌ 无 id 旧格式 `【此处插入XX图】`（下游无法定位）、用中文图题当 id `【此处插入:图表:系统总体架构图】`、空泛 slug `【此处插入:图表:tu1】`
- 同一张图（如"系统总体架构图"）即使跨文件引用，**全项目只登记一个 slug**，不重复造 id
...
#### 4.2.1 功能截图占位符预留（软件项目必备）

对于**软件项目**（采购内容包含系统/平台/应用开发），在编写功能方案章节时，**必须为每个将生成原型的系统的核心功能点预留系统原型截图占位符**——每系统 **3-6 个**，优先 ▲/★ 标注项与 P1 核心功能点。**不要为该系统的全部功能点都预留**：占位符过多会让下游 `bid-poc` 无法逐一实现为独立视图，导致大面积 pending（已发生过：某系统建 14 个占位符却只实现 5 个视图）。下游 `bid-poc` 按这些占位符生成原型页面，`bid-poc-screenshots` 读对照表自动截取并按 id 替换；**生成了视图但无占位符的孤儿截图由 bid-poc-screenshots §2.5 兜底补插**，但上游应尽量保证核心功能点都有占位符。

**措辞规则（评委可见，强制）**：标书正文中提到这些截图时，一律称"**系统原型**"或"**系统原型截图**"（如"下图为 XX 系统的原型界面"），**禁止出现 "POC"、"概念验证"、"Demo" 字样**——这些词在招投标语境里暗示方案不成熟。占位符周围的引导语、图注、小节标题同样适用此规则。

> 本节遵循**占位符对照表机制契约 v1**（详见 `packages/bidsmart-skills/CLAUDE.md` 的 "Placeholder registry" 段）。截图占位符对应 `type:"screenshot"`。**图表占位符（`type:"diagram"`）不在本节范围**，由 4.2 节"图表生成规范"处理，本节不得改写图表占位符。
...
其中 `<FP-id>` = **该功能点在 `system_decomposition.json` 里的 `id`**（如 `S01-001`）。截图占位符的 id **直接复用功能点 id——已唯一，禁止另造 id、禁止用系统编号或系统名文字充当 id**：
- ✅ `【此处插入:截图:S01-001】`
- ✅ `【此处插入:截图:S04-003】`
- ❌ `【此处插入:截图:S04性能预报系统】`（旧格式：系统标识文字，不是功能点 id）
- ❌ `【此处插入S04性能预报系统功能截图】`（旧"系统级"格式，已废弃）
- ❌ `【此处插入功能截图】`（无 id，下游无法定位）

> 旧的"系统级"占位符 `【此处插入{系统标识}功能截图】`（一个系统一个占位符）已废弃——新契约按**功能点**粒度预留（一个功能点一个占位符），id 取自 `system_decomposition.json` 的 `functionPoints[].id`。

##### 步骤 1：读取功能点 id（单一事实源）

从工作目录根的 `system_decomposition.json`（由 bid-system-decomp 生成）读取功能点清单，**占位符 id 一律取 `functionPoints[].id`**：

```bash
# 列出每个功能点的 所属系统 / id / 名称，作为截图占位符 id 与对照表登记的依据
jq -r '.systems[] as $s | $s.functionPoints[]? | [$s.id, .id, .name] | @tsv' system_decomposition.json
```

- 系统编号 / 系统名称 / 功能点 id 三者均以 json 为准，不得自行编号或命名
- 只对 `system_decomposition.json` 中存在功能点 id 的功能点预留截图占位符
- **json 不存在时**（独立调用 / 非软件标的兜底）：不得自行造 id；在完成状态块中标注 `⚠️ system_decomposition.json 缺失，截图占位符未登记`，便于 bid-manager / bid-assembly 发现系统分解阶段被跳过

##### 步骤 2：按功能点放置占位符

每个功能点在该系统功能描述章节中都有对应的描述位置。占位符放在**该功能点描述的末尾**——写完该功能点的技术方案正文之后，进入下一个功能点之前：
...
- **禁止把"插入"改成同义动词**：禁止出现 `【此处展示:图表:…】` / `【此处预留:图表:…】` / `【此处添加:图表:…】` 等变体——下游匹配不到
- **禁止外层加 markdown 加粗**：禁止写成 `**【此处插入:图表:…】**`
- **禁止用中文图题或空泛字符当 slug**：slug 用小写英文 + 短横线（如 `sys-arch`），不得写 `【此处插入:图表:系统总体架构图】` 或 `【此处插入:图表:tu1】`

**自检（编写后必须执行）：**
```bash
# 1. 正文图表占位符数量
echo "正文图表占位数: $(grep -c '此处插入:图表:' 响应文件/*.md)"
# 2. 对照表里 diagram pending 数量
echo "对照表 diagram pending: $(jq '[.items[] | select(.type=="diagram" and .status=="pending")] | length' placeholders.json)"
# 3. 残留的无 id 旧格式 / 同义动变种 / 加粗占位符（命中数应为 0）
grep -nP '【此处插入[^:]*图】|【此处(展示|暂留|预留|添加|生成):?图表|【此处(展示|暂留|预留|添加|生成)[^:]*图】|\*\*【?此处插入' 响应文件/*.md
```
- 自检 1 与自检 2 必须相等；正文每个图表占位符的 slug 都应能在对照表找到对应 item（漏预留或漏登记都会导致下游不渲染）
- 自检 3 命中行必须改回 `【此处插入:图表:<slug>】` 标准格式并登记对照表
- 自检未归零不得结束本文件编写
...
##### 4.3.1 技术力量/资质证明型文件 编写范式（强制）

技术力量、资质证明类附件（如"技术力量说明""资质证书清单""主要技术人员构成"等）极易出现掩码凑行、强声明编造，必须严格按以下范式编写：

**编写规则（强制）：**
- **文件结构镜像招标（最高优先级，按招标写法来）**：严格按招标提供的格式模板结构生成——招标怎么写就怎么办。招标给了几个独立模板（如"技术力量1/2/3/4/5"五个独立文件、或"技术力量说明"一个文件）就生成几个对应文件，沿用招标的命名与分组；**不得自行合并多个模板为一个文件，也不得自行拆分**。评标按评分表"关联格式"逐项定位，文件结构须与之一致，否则整组可能失分。
- **公司具备某资质** → 填写真实编号 / 颁发日期 / 持证人或单位身份 + 预留扫描件占位符（`【此处插入XX资质扫描件】`）
- **公司不具备该资质，或编号/日期等数据未确认** → 整行删除，或仅保留一段中性说明（如"我司就该项的实际情况将按采购方要求提供书面证明，截至编写时数据尚在核实中"）
- **禁止使用掩码凑行**：禁止出现 `××××`、`XXXX`、`20××`、`—` 等掩码字符替代真实编号/日期
- **禁止无证据时的强声明**：在公司未实际参与、或数据未确认的情况下，禁止填写"主要起草单位""起草单位""参编单位"等暗示深度参与的强声明文字

**自检（编写后必须执行）：**
```bash
# 检索资质表中的掩码字符与无证据强声明，命中数应为 0
grep -nP '×{2,}|X{2,}|20×{1,}|主要起草单位|起草单位' 响应文件/*.md
```
- 命中行必须人工核实：确有真实数据 → 补全；无证据 → 删除该行或改写为中性说明
- 自检未归零不得结束本文件编写
...
- [ ] 响应说明无空泛描述（逐条抽检）
- [ ] 进度计划在交付期限内
- [ ] **质保期/响应时效/网点承诺（硬约束，非软约束）**：必须读取 `bid_strategy`（商务标编写产物，含质保期/响应时效/服务网点承诺），技术标售后/运维方案中所列质保期、响应时效、网点承诺必须与商务侧完全一致；若 `bid_strategy` 缺失，则取分析报告★标注的商务基线，且**不得自行追加任何加分承诺**（如擅自延长质保至5年）。任何不一致 → 本文件状态标 `FAILED`，交由 bid-manager 处理，不得静默发布
...
软件项目的技术方案**必须**在功能章节预留系统原型截图占位符，格式与放置规则见 [4.2.1 功能截图占位符预留](#421-功能截图占位符预留软件项目必备)。**正文措辞一律称"系统原型"，禁止 "POC" 字样**（规则见 4.2.1）。

占位符格式：`【此处插入:截图:<FP-id>】`，其中 `<FP-id>` = 该功能点在 `system_decomposition.json` 的 `id`（如 `S04-001`），放在该功能点描述正文末尾。

**每写一个截图占位符，必须同步向工作目录根的 `placeholders.json` 幂等 upsert 一条 `type:"screenshot"`、`status:"pending"` 的 item**（id = 功能点 id），否则下游 `bid-poc-screenshots` 不会截图。

实际的截图截取与替换由下游 `bid-poc-screenshots` 阶段自动完成（读 `placeholders.json`，按 id 精确定位替换），本阶段只负责**预留位置 + 登记对照表**。

### 图表处理

所有图表必须生成为：**占位符 + Mermaid 代码块**。占位符用对照表格式 `【此处插入:图表:<slug>】`（slug 取法、向 `placeholders.json` 登记 `type:"diagram"` item 的步骤详见 [4.2 图表生成规范](#图表生成规范)）。
...
每写一个图表占位符，同步向 `placeholders.json` 幂等 upsert 一条 `type:"diagram"`、`status:"pending"` 的 item（id = slug），否则下游 `bid-mermaid-diagrams` 不会渲染。
...
- 标题使用 `#` `##` `###` 层级
- 表格使用 Markdown 表格语法
- 图片占位：`【此处插入:图表:<slug>】`（slug 见 4.2 图表生成规范；每写一个须向 `placeholders.json` 登记 `type:"diagram"`）
- 截图占位：`【此处插入:截图:<FP-id>】`（<FP-id> = `system_decomposition.json` 功能点 id，详见 4.2.1；每写一个须向 `placeholders.json` 登记）
- 签章标记：`（盖章）` `（签字）`
- 每个文件开头：`# 附件N：标题`
...
技术响应表条目数: {N}
▲截图占位数: {N}（已预留 `【此处插入:截图:<FP-id>】` 占位符并登记进 placeholders.json，由下游 bid-poc + bid-poc-screenshots 自动完成截图和替换）
图表占位数: {N}（已预留 `【此处插入:图表:<slug>】` 占位符+Mermaid代码并登记进 placeholders.json，由下游 bid-mermaid-diagrams 自动渲染）
评分子维度覆盖: {已覆盖}/{总数}
输出目录: 响应文件/
状态: SUCCESS
--- END ---
...
**重要——待补充材料清单分类**：完成摘要后可列出仍需补充的材料，但**必须区分两类**：

**🔧 流水线自动处理（本阶段无需关注）：**
- ✅ 系统原型功能截图 → 下游 `bid-poc` + `bid-poc-screenshots` 自动生成并替换
- ✅ Mermaid 图表 → 下游 `bid-mermaid-diagrams` 自动渲染为 PNG
- ✅ 扫描件（资料库已有） → 下游 `bid-material-search` 自动替换

**⚠️ 需要人工补充（正式投标前完成）：**
- 证书扫描件（营业执照、资质证书、专利证书等）
- 证明材料（服务网点证明、合同案例等）
- 签章/签字

**禁止**将原型截图或图表列在"需要人工补充"清单中——这些是流水线自动完成的。如果混在一起，用户会困惑为什么"还在提示需要截图"。
