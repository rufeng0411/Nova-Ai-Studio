...
- Output location: `artifacts/podcast/` under the workspace (create if missing)
- Prefer **Wonda** (`create-wonda`) when Volcengine TTS env is not configured

### Step 2: Create Structured Script JSON

Generate a structured JSON script file at `artifacts/podcast/{descriptive-name}-script.json`
...
Call the Python script (paths relative to workspace root):

```bash
python skills/vendor/deer-flow/df-podcast-generation/scripts/generate.py \
  --script-file artifacts/podcast/script-file.json \
  --output-file artifacts/podcast/generated-podcast.mp3 \
  --transcript-file artifacts/podcast/generated-podcast-transcript.md
```
...
Step 1: Create script file `artifacts/podcast/ai-history-script.json`:
```json
{
  "title": "The History of Artificial Intelligence",
  "locale": "en",
...
```bash
python skills/vendor/deer-flow/df-podcast-generation/scripts/generate.py \
  --script-file artifacts/podcast/ai-history-script.json \
  --output-file artifacts/podcast/ai-history-podcast.mp3 \
  --transcript-file artifacts/podcast/ai-history-transcript.md
```
...
- Podcasts and transcripts are saved under `artifacts/podcast/`
- Share both the podcast MP3 and transcript MD paths in the reply
