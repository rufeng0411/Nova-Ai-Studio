...
- HTML 放映稿写入 **`<task-artifact-dir>/index.html`**（仅用户明确要 HTML 幻灯/网页放映时）。
- 用户要可编辑 `.pptx` 时改 read_skill anth-pptx 或 cyber-ppt，**勿**用 HTML 充数。
- **禁止**无 theme/layout/CSS token 的裸 HTML 幻灯；**禁止白板式**纯标题+列表页。
- **禁止** ask_user_question 挡 write_file。
<!-- NOVA-EXEC-END -->
