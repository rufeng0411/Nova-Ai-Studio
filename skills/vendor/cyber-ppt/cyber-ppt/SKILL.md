...
- 工作目录：仅 **`<task-artifact-dir>/`**。
- **终态必交付**（缺一则 incomplete）：
  - `《{主题}》咨询汇报.pptx`（alias: `presentation.pptx`）
  - `slide_manifest.json`、`visual_qa_gate.json`（JSON 保持英文）
- **Hub「试一下」/ 用户已给主题+页数时**：三阶段确认门改为 **内部自检门**——用推荐 SCR/默认 Cyber 风格 #3 直接推进，**禁止** `ask_user_question` 阻塞 write_file。
- 仅当用户 goal 含「先给我看大纲/先选风格」时，在对话正文展示大纲并等待下一轮用户消息（仍禁止 ask_user 工具）。
- 阶段 3 须 `python-pptx` 或 `export_document` 产出可编辑 pptx；禁止 HTML/纯 Markdown/脚本充数。
- **Hub 试一下 / 已给主题+页数**：必读 `references/nova-fast-path.md` + `references/nova-ppt-visual-minimum.md`；**禁止** ImageGen 全页蓝图、禁止 8 风格样张、禁止 Task/subagent；**禁止白板/无风格/无布局 pptx**；**禁止画布越界**（须 `nova_pptx_layout` / `nova_build_from_manifest.py`，13.333×7.5 in）；主线程交付 pptx+manifest+qa_gate。
<!-- NOVA-EXEC-END -->
...
将源材料转化为有证据链、结论先行、可编辑的咨询式演示文稿。使用 MBB 级标准审视证据、论证和页面架构。保留证据可追溯性，并以渲染结果而不是文件生成作为完成判断。

## 强制流程（Nova 自动交付模式）

| 阶段 | 必须产出 | Nova 停止条件（非 ask_user） | 读取 |
|---|---|---|---|
| 1. 分析 | MBB 标准证据表、SCR 论证、逐页大纲（可写 `<task-artifact-dir>/outline.md`） | 大纲写入后即可进入阶段 2 | `references/source-analysis.md`, `references/storyline.md` |
| 2. 蓝图 | 8 种视觉风格、选定风格（默认 #3）、逐页 ImageGen 蓝图（`<task-artifact-dir>/blueprints/`） | 蓝图就绪即可进入阶段 3 | `references/visual-system.md` |
| 3. 混合还原 PPTX | `《{主题}》咨询汇报.pptx` + `slide_manifest.json` + `visual_qa_gate.json` + 全页渲染图 | 三文件 write 完成即终态 | `references/ppt-production.md`, `references/quality-assurance.md` |

**Nova 默认**：未经用户明确要求「先看大纲/选风格」时，**禁止** `ask_user_question` 跨阶段确认；用推荐故事线与默认风格自动推进至 pptx 落盘。用户要求修改时，回到对应阶段修订并继续（仍禁止 ask_user 工具挡 write_file）。
