...
- 可编辑 pptx 写入 **`<task-artifact-dir>/《{主题}》演示文稿.pptx`**（alias: `presentation.pptx`）。
- **禁止** ask_user_question 挡 write_file；禁止 HTML 充数；还原后 pptx 须保留源图版式密度，**禁止**退化为白板 bullet 页。
<!-- NOVA-EXEC-END -->
