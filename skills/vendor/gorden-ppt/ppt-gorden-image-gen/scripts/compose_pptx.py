...
describing every slide as stacked layers (back to front):
  background (full-bleed) -> frame structure (full-bleed transparent) ->
  shapes (native cards/lines) -> icon/decoration PNGs (positioned) -> text boxes
and writes a real PowerPoint file where text stays editable and icons/frame stay
as movable pictures.
...
Layout schema: see references/image-to-pptx.md. Quick form:
{
  ...
  "ref_width": 2048, "ref_height": 1152,   // required when units == "px"
  "assets_dir": ".",              // base dir for relative file paths
  "slides": [
    {
      "background": "01-bg.png",
      "frame": "01-frame.png",        // optional full-bleed transparent skeleton
      "icons": [{"file": "icons/i_r1c1.png", "x":0.1,"y":0.2,"w":0.08,"h":0.08}],
      "texts": [{"text":"标题","x":0.08,"y":0.05,"w":0.6,"h":0.12,
                 "size":40,"color":"#1A1A1A","bold":true,"align":"left","valign":"top",
...
def build_pptx(deck, out_path: Path):
    ...
    for idx, sl in enumerate(deck["slides"], 1):
        ...
        for tx in sl.get("texts", []):
            ...
            size_pt = tx.get("size")
            if size_pt is None and tx.get("size_px") is not None and ref_h:
                size_pt = float(tx["size_px"]) * sh_pt / ref_h
            size_pt = float(size_pt) if size_pt else 18.0
            ...
            if runs:
                ...
                for rinfo in runs:
                    ...
                    run.font.size = Pt(float(rinfo.get("size", size_pt)))
...
def render_previews(deck, preview_dir: Path):
    ...
    for idx, sl in enumerate(deck["slides"], 1):
        ...
        for tx in sl.get("texts", []):
            ...
            fw = _frac(deck, tx, "w", "w", ref_w)
            size_pt = tx.get("size")
            if size_pt is None and tx.get("size_px") is not None:
                size_pt = float(tx["size_px"]) * sh_pt / ref_h
            size_pt = float(size_pt) if size_pt else 18.0
            ...
            if runs:
                ...
                for rinfo in runs:
                    ...
                    rpx = max(8, int(round(float(rinfo.get("size", size_pt)) * CH / sh_pt)))
                    ...
                    segs.append((rtext, rfont, rinfo.get("color", color), rb, w))
                    total += w
                ...
                for rtext, rfont, rcolor, rb, w in segs:
                    stroke = 1 if rb and not bold_path else 0
                    draw.text((cx, ty), rtext, fill=rcolor, font=rfont,
                              stroke_width=stroke, stroke_fill=rcolor)
                    cx += w
            else:
                ...
                for li, line in enumerate(lines):
                    ...
                    draw.text((max(bx0, int(lx)), ty + li * line_h), line, fill=color,
                              font=font, stroke_width=stroke, stroke_fill=color)
