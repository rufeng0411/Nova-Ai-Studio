...
function normalizeSkillAssetRelativePath(raw) {
...
// PD-SAAS-FORK: read bundled skill reference files (e.g. diagram-maker/references/svg-template.md)
router.post('/read-asset', async (req, res) => {
  try {
    const { slug, relativePath, projectPath } = req.body || {};
    if (!safeSlug(slug)) {
      return res.status(400).json({ error: 'invalid skill slug' });
    }
    const rel = normalizeSkillAssetRelativePath(relativePath);
    if (!rel) {
      return res.status(400).json({ error: 'invalid relativePath' });
    }

    const candidates = [
      path.join(userSkillsRoot(), slug, rel),
      path.join(REPO_ROOT, 'skills', slug, rel),
      path.join(REPO_ROOT, 'skills', 'vendor', slug, rel),
    ];
    if (projectPath && !isGeneralCwd(projectPath)) {
      candidates.unshift(path.join(projectSkillsRoot(projectPath), slug, rel));
    }
    ...
    return res.status(404).json({ error: 'Skill asset not found' });
  } catch (e) {
    sendGatewayError(res, e);
  }
});
