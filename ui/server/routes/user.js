...
function spawnAsync(command, args, options = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(command, args, withHiddenConsole({ ...options, shell: false }));
    let stdout = '';
    let stderr = '';
...
router.get('/onboarding-status', authenticateToken, async (req, res) => {
  try {
    // PD-SAAS-FORK: SaaS 由平台管理员配置模型池，成员无需 OSS 引导
    const hasCompleted = isSaasMode() || hasUsablePilotDeckConfig();
