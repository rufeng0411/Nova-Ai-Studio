...
import { pluginsListKey } from '../saas/cache/cacheKeys.js';
...
router.get('/', async (req, res) => {
  try {
    const cacheKey = pluginsListKey();
    const cached = await cacheGet(cacheKey);
    if (cached && typeof cached === 'object' && Array.isArray(cached.plugins)) {
    ...
    void cacheSet(cacheKey, payload, getDefaultTtl('plugins'));
    res.json(payload);
  } catch (err) {
