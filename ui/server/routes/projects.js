...
import { spawn } from '../utils/childProcess.js';
import { withHiddenConsole } from '../utils/winSpawn.js';
import os from 'os';
import {
  addProjectManually,
...
import { createSaasNamedWorkspace } from '../saas/storage/fileStorageService.js';
import { invalidateProjectsListCache } from '../saas/conversation/catalogCache.js';
import { resolvePathInProject } from '../utils/pathInProject.js';
import {
...
router.post('/create-workspace', async (req, res) => {
  try {
    ...
    // PD-SAAS-FORK: SaaS cloud-only — name-only workspace creation.
    if (isSaasMode() && getSaasRequestContext()?.userId) {
      if (workspaceType === 'existing' || workspacePath) {
        return res.status(403).json({
          error: 'SaaS workspaces are stored in the cloud. Provide displayName only.',
        });
      }
      ...
      const project = await createSaasNamedWorkspace(label);
      await invalidateProjectsListCache(req.user);
      return res.json({
        success: true,
        project,
        message: 'SaaS workspace created successfully',
      });
    }
...
router.get('/clone-progress', async (req, res) => {
  ...
  try {
    ...
    const gitProcess = spawn('git', ['clone', '--progress', cloneUrl, clonePath], withHiddenConsole({
...
function cloneGitHubRepository(githubUrl, destinationPath, githubToken = null) {
  return new Promise((resolve, reject) => {
    ...
    const gitProcess = spawn('git', ['clone', '--progress', cloneUrl, destinationPath], withHiddenConsole({
      stdio: ['ignore', 'pipe', 'pipe'],
      env: {
        ...process.env,
        GIT_TERMINAL_PROMPT: '0'
...
function spawnRevealInOs(targetPath) {
  ...
  const child = spawn(command, args, withHiddenConsole({ stdio: 'ignore', detached: true }));
  child.unref();
}

router.post('/:projectName/reveal', async (req, res) => {
  try {
    ...
    const resolvedResult = resolvePathInProject(projectRoot, targetPath);
    if (!resolvedResult.valid) {
      return res.status(403).json({ success: false, error: resolvedResult.error });
    }

    const revealPath = resolvedResult.resolved;
    const stats = await fs.stat(revealPath).catch(() => null);
    if (!stats) {
    ...
    if (stats.isDirectory()) {
      if (process.platform === 'win32') {
        const child = spawn('explorer.exe', [revealPath], withHiddenConsole({ stdio: 'ignore', detached: true }));
        child.unref();
      } else if (process.platform === 'darwin') {
        const child = spawn('open', [revealPath], withHiddenConsole({ stdio: 'ignore', detached: true }));
        child.unref();
      } else {
        const child = spawn('xdg-open', [revealPath], withHiddenConsole({ stdio: 'ignore', detached: true }));
        child.unref();
      }
      return res.json({ success: true, path: revealPath });
    }

    spawnRevealInOs(revealPath);
    res.json({ success: true, path: revealPath });
  } catch (error) {
    res.status(500).json({
      success: false,
