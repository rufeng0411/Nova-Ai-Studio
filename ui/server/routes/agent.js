...
async function getGitRemoteUrl(repoPath) {
  return new Promise((resolve, reject) => {
    const gitProcess = spawn('git', ['config', '--get', 'remote.origin.url'], withHiddenConsole({
      cwd: repoPath,
...
async function getCommitMessages(projectPath, limit = 5) {
  return new Promise((resolve, reject) => {
    const gitProcess = spawn('git', ['log', `-${limit}`, '--pretty=format:%s'], withHiddenConsole({
      cwd: projectPath,
      stdio: ['pipe', 'pipe', 'pipe']
    }));
...
async function cloneGitHubRepo(githubUrl, githubToken = null, projectPath) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Execute git clone
      const gitProcess = spawn('git', ['clone', '--depth', '1', cloneUrl, cloneDir], withHiddenConsole({
        stdio: ['pipe', 'pipe', 'pipe']
      }));
    ...
    if (createBranch || createPR) {
      try {
        ...
        if (createBranch) {
          ...
          const checkoutProcess = spawn('git', ['checkout', '-b', finalBranchName], withHiddenConsole({
            cwd: finalProjectPath,
            stdio: 'pipe'
          }));
            ...
            checkoutProcess.on('close', (code) => {
              if (code === 0) {
                ...
                if (stderr.includes('already exists')) {
                  ...
                  const checkoutExisting = spawn('git', ['checkout', finalBranchName], withHiddenConsole({
                    cwd: finalProjectPath,
                    stdio: 'pipe'
                  }));
          ...
          const pushProcess = spawn('git', ['push', '-u', 'origin', finalBranchName], withHiddenConsole({
            cwd: finalProjectPath,
            stdio: 'pipe'
          }));
        ...
        if (createPR) {
          ...
          prBody += '\n\n---\n*This pull request was automatically created by Nova Ai-Studio Agent.*';
