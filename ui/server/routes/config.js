...
// PD-SAAS-FORK: SaaS non-admin cannot mutate platform config (requireSaasConfigWrite).
import { requireSaasConfigWrite } from '../saas/middleware/requireSaasConfigWrite.js';
...
router.put('/', requireSaasConfigWrite, async (req, res) => {
  try {
...
router.post('/reload', requireSaasConfigWrite, async (_req, res) => {
  try {
    const record = readPilotDeckConfigFile();
    const validation = validatePilotDeckConfig(record.config);
    if (!validation.valid) {
...
router.post('/test-connection', requireSaasConfigWrite, async (req, res) => {
 ...
 * `src/tool/builtin/webSearch.ts`'s GLM/Tavily/Bocha/custom request shape. Returns:
 * `{ ok, error?, latencyMs?, organicCount? }` to match the convention
 * established by `/test-connection`.
 */
router.post('/test-web-search', requireSaasConfigWrite, async (req, res) => {
  const { provider, apiKey, endpoint, customProvider } = req.body || {};
  const selectedProvider =
    provider === 'tavily' || provider === 'bocha' || provider === 'custom' ? provider : 'glm';
  ...
  try {
    ...
    } else if (selectedProvider === 'bocha') {
      requestUrl = effectiveEndpoint;
      requestInit = {
        method: 'POST',
        headers: {
...
router.post('/open', requireSaasConfigWrite, async (_req, res) => {
