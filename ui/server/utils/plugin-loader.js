...
/** PD-SAAS-FORK: isolated install — avoid monorepo npm overrides (EOVERRIDE on react) */
function installPluginDependencies(cwd) {
  return new Promise((resolve, reject) => {
    const packageManager = process.platform === 'win32' ? 'pnpm.cmd' : 'pnpm';
    const tryPnpm = spawn(packageManager, ['install', '--ignore-scripts'], withHiddenConsole({
      cwd,
    ...
    let pnpmErr = '';
    tryPnpm.stderr.on('data', (data) => { pnpmErr += data.toString(); });
    tryPnpm.on('close', (code) => {
      if (code === 0) {
        resolve();
        return;
      }
      const npmCmd = process.platform === 'win32' ? 'npm.cmd' : 'npm';
      const npmProcess = spawn(npmCmd, ['install', '--ignore-scripts'], withHiddenConsole({
        cwd,
        stdio: ['ignore', 'pipe', 'pipe'],
        shell: false,
        env: {
      ...
      npmProcess.on('close', (npmCode) => {
        if (npmCode !== 0) {
          reject(new Error(`plugin dependency install failed (pnpm/npm exit ${code}/${npmCode}): ${pnpmErr.trim()}`));
          return;
        }
        resolve();
      });
    ...
    tryPnpm.on('error', () => {
      ...
      npmProcess.on('close', (npmCode) => {
        if (npmCode !== 0) {
          reject(new Error(`npm install failed (exit code ${npmCode})`));
          return;
        }
        resolve();
      });
...
function runBuildIfNeeded(dir, packageJsonPath, onSuccess, onError) {
  ...
  const npmCmd = process.platform === 'win32' ? 'npm.cmd' : 'npm';
  const buildProcess = spawn(npmCmd, ['run', 'build'], withHiddenConsole({
    cwd: dir,
    stdio: ['ignore', 'pipe', 'pipe'],
    shell: false,
  }));
  ...
  return new Promise((resolve, reject) => {
    ...
    gitProcess.on('close', (code) => {
      ...
      if (fs.existsSync(packageJsonPath)) {
        installPluginDependencies(tempDir)
          .then(() => runBuildIfNeeded(tempDir, packageJsonPath, () => finalize(manifest), (err) => { cleanupTemp(); reject(err); }))
          .catch((err) => { cleanupTemp(); reject(err); });
      } else {
        finalize(manifest);
      }
    });
    ...
    gitProcess.on('close', (code) => {
      ...
      if (fs.existsSync(packageJsonPath)) {
        installPluginDependencies(pluginDir)
          .then(() => runBuildIfNeeded(pluginDir, packageJsonPath, () => resolve(manifest), (err) => reject(err)))
