...
export const useCodeEditorDocument = ({ file, projectPath }: UseCodeEditorDocumentParams) => {
  ...
  const initialApiPath = resolveEditorApiPath(file.path, projectPath);
  const [filePath, setFilePath] = useState(initialApiPath);
  const fileDiffNewString = file.diffInfo?.new_string;
  const fileDiffOldString = file.diffInfo?.old_string;

  useEffect(() => {
    setFilePath(initialApiPath);
  }, [initialApiPath]);
  ...
  useEffect(() => {
    ...
    const loadFileContent = async () => {
      try {
        ...
        const skillAsset = parseSkillAssetEditorPath(file.path);
        if (skillAsset) {
          const response = await api.readSkillAsset(
            skillAsset.slug,
            skillAsset.relativePath,
            projectPath,
          );
          if (!response.ok) {
        ...
        const resolved = await resolveProjectFilePath(fileProjectName, filePath, projectPath);
        if (resolved?.relativePath) {
          apiPath = resolved.relativePath;
          if (!cancelled) {
            setFilePath(apiPath);
          }
        }

        const response = await api.readFile(fileProjectName, apiPath);
        if (!response.ok) {
          throw new Error(`Failed to load file: ${response.status} ${response.statusText}`);
        }
  ...
  }, [file.diffInfo, file.name, file.path, fileDiffNewString, fileDiffOldString, filePath, fileProjectName, projectPath, reloadToken]);
  ...
  const handleSave = useCallback(async () => {
    ...
    if (parseSkillAssetEditorPath(file.path)) {
      setSaveError('技能参考文件为只读，无法保存到项目目录');
      return;
    }
