...
type SkillsV2Props = {
 ...
 projects: Project[];
 /** PD-SAAS-FORK: 后台右栏 — 仅编辑器，由能力中心卡片驱动选中 */
 panelMode?: boolean;
 /** PD-SAAS-FORK: 后台使用页头「新建技能」，隐藏右栏新建 */
 panelHideNew?: boolean;
...
export default function SkillsV2({
 ...
 useEffect(() => {
 if (!panelMode || externalSlug === undefined) return;
 if (!externalSlug) {
 setActiveSlug(null);
 setActiveScope(null);
 return;
 ...
 if (!skills) return;
 const projectSkill = skills.project.find((s) => s.slug === externalSlug);
 const userSkill = skills.user.find((s) => s.slug === externalSlug);
 const next = projectSkill
 ? { slug: externalSlug, scope: 'project' as const }
 : userSkill
 ? { slug: externalSlug, scope: 'user' as const }
 : null;
 if (next) {
 ...
 <span className="text-[12px] font-medium text-foreground">技能内容</span>
 <div className="flex items-center gap-1">
 <button
 type="button"
 onClick={refresh}
 disabled={loading}
 className="inline-flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground transition hover:bg-muted disabled:opacity-50"
 ...
 className="inline-flex h-7 items-center gap-1.5 rounded-md bg-primary px-2.5 text-[12px] font-medium text-primary-foreground"
 >
 ...
 <div className={cn('flex min-h-0 flex-1 flex-col', !panelMode && 'border-l border-border')}>
 {activeSkill ? (
 <SkillDetail
 skill={activeSkill}
 content={editorContent}
 ...
 t={t}
 />
 ) : panelMode && catalogFallback && externalSlug === catalogFallback.slug ? (
 <CatalogSkillPlaceholder fallback={catalogFallback} />
 ) : (
 <EmptyState t={t} panelMode={panelMode} />
 )}
 </div>
 </div>
 ...
 toast.kind === 'success' && 'bg-emerald-600 text-primary-foreground',
...
function Header({
 ...
 <div className="flex min-w-0 items-center gap-2 truncate font-mono text-xxs text-muted-foreground">
 ...
 type="button"
 onClick={onNew}
 className="inline-flex h-7 items-center gap-1.5 rounded-md bg-primary px-2.5 text-[12px] font-medium text-primary-foreground transition hover:bg-neutral-700 dark:bg-muted dark:text-foreground dark:hover:bg-neutral-300"
 >
 <Plus className="h-3.5 w-3.5" strokeWidth={1.75} />
 <span>{t('skillsTab.newSkill', { defaultValue: 'New' })}</span>
 </button>
...
function SkillsList({
 ...
 <div className="px-4 py-6 text-center text-xxs text-muted-foreground">
...
function ListSection({
 ...
 return (
 <div className="mb-2">
 <div className="px-4 py-1 text-xxs uppercase tracking-wider text-muted-foreground">
 ...
 : 'text-foreground hover:bg-sidebar dark:hover:bg-primary/60',
 )}
 ...
 {s.version ? (
 <span className="shrink-0 rounded bg-border px-1 py-px text-[10px] text-muted-foreground">
 v{s.version}
 </span>
 ) : null}
 </div>
 {s.description ? (
 <div className="mt-0.5 line-clamp-1 text-xxs text-muted-foreground">
 {s.description}
 </div>
 ) : null}
 </button>
 ...
 className="absolute right-1.5 top-1/2 hidden h-6 w-6 -translate-y-1/2 items-center justify-center rounded text-muted-foreground hover:bg-red-50 hover:text-red-600 group-hover:inline-flex dark:hover:bg-red-950/40 dark:hover:text-red-400"
 ...
 <div
 ref={menuRef}
 className="fixed z-[100] min-w-[180px] rounded-lg border border-border bg-card py-1 shadow-xl"
 ...
 className="flex w-full items-center justify-between gap-2 px-3 py-1.5 text-left text-[12px] text-foreground hover:bg-muted"
 >
 ...
 {showMoveSubmenu ? (
 <div
 className="absolute left-full top-0 z-[101] ml-1 min-w-[160px] max-h-[240px] overflow-y-auto rounded-lg border border-border bg-card py-1 shadow-xl"
 >
...
function EmptyState({
 ...
 <Sparkles className="h-8 w-8 text-muted-foreground dark:text-foreground" strokeWidth={1.5} />
 <div>
 {panelMode
 ? '点击左侧能力卡片，在此查看或编辑 SKILL.md'
 : t('skillsTab.selectHint', { defaultValue: 'Pick a skill on the left to view or edit its SKILL.md.' })}
 </div>
 </div>
 );
}

function CatalogSkillPlaceholder({
 fallback,
}: {
 fallback: NonNullable<SkillsV2Props['catalogFallback']>;
}) {
 return (
 <div className="flex h-full flex-col items-center justify-center gap-3 px-6 text-center text-[13px] text-muted-foreground">
 <FolderSearch className="h-8 w-8 text-muted-foreground" strokeWidth={1.5} />
 <div className="max-w-sm space-y-2">
 <p className="font-medium text-foreground">{fallback.name}</p>
 <p>该能力在目录中，尚未安装到当前运行时。可在上方「新建」从仓库安装，或检查 vendor 包是否已同步。</p>
 {fallback.source ? (
 <p className="truncate font-mono text-[11px] text-muted-foreground">{fallback.source}</p>
 ) : null}
 </div>
 </div>
 );
...
function SkillDetail({
 ...
 skill.scope === 'project'
 ? 'bg-blue-100 text-info dark:bg-blue-950/60'
 : 'bg-amber-100 text-warning /60',
 )}
 >
 {skill.scope}
 </span>
 {skill.version ? (
 <span className="text-xxs text-muted-foreground">v{skill.version}</span>
 ) : null}
 </div>
 {skill.description ? (
 <p className="mt-1 text-xxs text-muted-foreground">{skill.description}</p>
 ) : null}
 ...
 {loading ? (
 <div className="flex h-full items-center justify-center gap-2 text-xxs text-muted-foreground">
 <Loader2 className="h-3.5 w-3.5 animate-spin" strokeWidth={1.75} />
 <span>{t('skillsTab.loading', { defaultValue: 'Loading…' })}</span>
 </div>
 ) : (
 ...
 type="button"
 onClick={onRevert}
 className="inline-flex h-7 items-center rounded-md px-2.5 text-[12px] text-muted-foreground hover:bg-muted dark:hover:bg-primary"
 >
 ...
 className="inline-flex h-7 items-center gap-1.5 rounded-md bg-primary px-2.5 text-[12px] font-medium text-primary-foreground transition hover:bg-neutral-700 disabled:opacity-40 dark:bg-muted dark:text-foreground dark:hover:bg-neutral-300"
 >
...
function NewSkillModal({
 ...
 <div className="flex h-[560px] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-border bg-card shadow-2xl">
 <div className="flex shrink-0 items-center justify-between border-b border-border px-5 py-3">
 <h3 className="text-sm font-semibold">{t('skillsTab.newTitle', { defaultValue: 'New Skill' })}</h3>
 <button
 type="button"
 onClick={onClose}
 className="inline-flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground hover:bg-muted dark:hover:bg-primary"
...
function ModalTab({
 ...
 active
 ? 'border-neutral-900 font-medium text-foreground dark:border-border'
...
function ScopeSelector({
 ...
 : 'text-muted-foreground hover:bg-sidebar dark:hover:bg-primary',
 )}
 >
 ...
 scope === 'project'
 ? 'bg-primary text-primary-foreground dark:bg-muted dark:text-foreground'
 : 'text-muted-foreground hover:bg-sidebar disabled:opacity-40 disabled:hover:bg-transparent dark:hover:bg-primary',
 )}
 >
...
function InstallFromClawHub({
 ...
 <Search className="absolute left-2.5 h-3.5 w-3.5 text-muted-foreground" strokeWidth={1.75} />
 <input
 type="text"
 value={query}
 onChange={(e) => setQuery(e.target.value)}
 placeholder={t('skillsTab.searchPlaceholder', { defaultValue: 'Search clawhub.com…' }) as string}
 className="h-8 w-full rounded-md border border-border bg-card pl-8 pr-2 text-[13px] outline-none focus:border-ring"
 />
 {searching ? (
 <Loader2 className="absolute right-2.5 h-3.5 w-3.5 animate-spin text-muted-foreground" strokeWidth={1.75} />
 ) : null}
 </div>
 ...
 <div className="flex h-full items-center justify-center text-xxs text-muted-foreground">
 {query.trim()
 ...
 <span className="font-mono text-[11px] text-muted-foreground">{r.slug}</span>
 {r.score ? (
 <span className="text-[10px] text-muted-foreground">· {r.score.toFixed(2)}</span>
 ) : null}
 </div>
 </div>
 <button
 ...
 className={cn(
 'inline-flex h-7 shrink-0 items-center gap-1.5 rounded-md px-2.5 text-[12px] font-medium text-primary-foreground transition disabled:opacity-50',
 isForce ? 'bg-amber-600 hover:bg-warning/100' : 'bg-primary hover:bg-neutral-700 dark:bg-muted dark:text-foreground dark:hover:bg-neutral-300',
 )}
 >
 {isInstalling ? (
 <Loader2 className="h-3.5 w-3.5 animate-spin" strokeWidth={1.75} />
...
function ImportFromFolder({
 ...
 batchMode
 ? 'cursor-not-allowed border-border text-muted-foreground dark:border-neutral-900'
 : 'border-border bg-card text-foreground hover:bg-sidebar dark:hover:bg-primary',
 )}
 >
 ...
 className={cn(
 'h-8 flex-1 rounded-md border bg-card px-2 font-mono text-[12px] outline-none focus:border-ring',
 (picked || batchMode)
 ? 'cursor-not-allowed border-border text-muted-foreground dark:border-neutral-900'
 : 'border-border ',
 )}
 ...
 {picked ? (
 <div className="mt-2 flex items-center gap-2 rounded-md bg-muted px-2.5 py-1.5 text-[12px]">
 ...
 type="button"
 onClick={clearPicked}
 className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded text-muted-foreground hover:bg-border"
 >
 ...
 className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded text-muted-foreground hover:bg-border disabled:opacity-40"
 >
 <X className="h-3 w-3" strokeWidth={1.75} />
 </button>
 </div>
 ...
 {/* Select all */}
 {!batchDone && (
 <div className="border-b border-border px-3 py-1.5 dark:border-neutral-900">
 ...
 {/* Progress header */}
 {batchImporting && (
 <div className="border-b border-border px-3 py-1.5 text-[11px] text-muted-foreground dark:border-neutral-900">
 ...
 {batchDone && (
 <div className="border-b border-border px-3 py-1.5 text-[11px] font-medium dark:border-neutral-900">
 ...
 {result.status === 'success' && <CheckCircle2 className="h-3.5 w-3.5 text-success dark:text-emerald-500" strokeWidth={1.75} />}
 {result.status === 'error' && <XCircle className="h-3.5 w-3.5 text-red-600 dark:text-red-500" strokeWidth={1.75} />}
 {result.status === 'importing' && <Loader2 className="h-3.5 w-3.5 animate-spin text-muted-foreground" strokeWidth={1.75} />}
 ...
 <div className="flex items-center gap-1.5">
 <Folder className={cn('h-3 w-3 shrink-0', isSkill ? 'text-amber-500' : 'text-muted-foreground dark:text-foreground')} strokeWidth={1.75} />
 <span className={cn('truncate text-[12px]', isSkill ? 'font-medium' : 'text-muted-foreground')}>
 ...
 className={cn(
 'h-8 w-full rounded-md border bg-card px-2 font-mono text-[12px] outline-none',
 slugValid
 ...
 className="inline-flex h-8 items-center gap-1.5 rounded-md bg-primary px-3 text-[12px] font-medium text-primary-foreground transition hover:bg-neutral-700 dark:bg-muted dark:text-foreground dark:hover:bg-neutral-300"
 >
 ...
 className="inline-flex h-8 items-center gap-1.5 rounded-md bg-primary px-3 text-[12px] font-medium text-primary-foreground transition hover:bg-neutral-700 disabled:opacity-40 dark:bg-muted dark:text-foreground dark:hover:bg-neutral-300"
 >
 {batchImporting ? (
 <Loader2 className="h-3.5 w-3.5 animate-spin" strokeWidth={1.75} />
 ) : (
...
function ValidationPanel({
 ...
 {validating ? (
 <Loader2 className="ml-auto h-3.5 w-3.5 animate-spin text-muted-foreground" strokeWidth={1.75} />
 ) : result?.ok ? (
 <CheckCircle2 className="ml-auto h-3.5 w-3.5 text-success dark:text-emerald-500" strokeWidth={1.75} />
 ) : (
...
function CreateFromScratch({
 ...
 placeholder="my-skill"
 className={cn(
 'h-8 w-full rounded-md border bg-card px-2 font-mono text-[13px] outline-none',
 slugValid || !slug
 ? 'border-border focus:border-ring'
 : 'border-red-300 dark:border-red-800',
 )}
 ...
 className="h-8 w-full rounded-md border border-border bg-card px-2 text-[13px] outline-none focus:border-ring"
 />
 </Field>
 </div>
 ...
 rows={2}
 className="w-full rounded-md border border-border bg-card px-2 py-1.5 text-[13px] outline-none focus:border-ring"
 />
 </Field>
 ...
 rows={8}
 className="w-full rounded-md border border-border bg-card px-2 py-1.5 font-mono text-[12px] outline-none focus:border-ring"
...
function Field({
 ...
 {children}
 {hint ? <p className="mt-1 text-xxs text-muted-foreground">{hint}</p> : null}
 </div>
 );
}
