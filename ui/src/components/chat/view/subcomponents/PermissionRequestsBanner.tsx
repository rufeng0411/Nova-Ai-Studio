...
export default function PermissionRequestsBanner({
 ...
 <div
 key={entry}
 className="rounded-lg border border-warning/30 bg-warning/10 p-3 shadow-sm dark:bg-amber-900/20"
 >
 ...
 <summary className="cursor-pointer text-xs text-warning hover:text-warning dark:hover:text-amber-100">
 View tool input
 </summary>
 <pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap rounded-md border border-warning/30 bg-card/80 p-2 text-xs text-warning /60">
 {rawInput}
 </pre>
 </details>
 )}
 ...
 return inp ? (
 <pre key={r.requestId} className="max-h-28 overflow-auto whitespace-pre-wrap rounded-md border border-warning/30 bg-card/80 p-2 text-xs text-warning /60">
 {inp}
 </pre>
 ) : null;
 })}
 ...
 className="inline-flex items-center gap-2 rounded-md bg-amber-600 px-3 py-1.5 text-xs font-medium text-primary-foreground transition-colors hover:bg-amber-700"
 >
 Allow once
 </button>
 <button
 ...
 permissionEntry
 ? 'border-amber-300 text-warning hover:bg-amber-100 dark:border-amber-700 dark:hover:bg-amber-900/30'
