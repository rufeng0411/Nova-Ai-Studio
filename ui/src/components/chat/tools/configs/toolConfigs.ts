import { extractDeliverablePath } from '../../../../shared/artifactPaths';
...
export interface ToolDisplayConfig {
  ...
  result?: {
    ...
    contentType?: 'markdown' | 'file-list' | 'artifact-deliverable' | 'todo-list' | 'text' | 'success-message' | 'task' | 'question-answer' | 'plan-card';
...
export const TOOL_CONFIGS: Record<string, ToolDisplayConfig> = {
  ...
  Edit: {
    ...
    result: {
      type: 'collapsible',
      defaultOpen: true,
      contentType: 'artifact-deliverable',
      getContentProps: (result: any, helpers?: { selectedProject?: unknown; onFileOpen?: (p: string) => void; toolInput?: unknown }) => ({
        filePath: extractDeliverablePath(result, helpers?.toolInput),
        result,
        toolInput: helpers?.toolInput,
  ...
  render_html_video: {
    input: {
      type: 'one-line',
      label: 'HTML 转视频',
      getValue: (input) => input.html_path || input.htmlPath || '',
      action: 'open-file',
    },
    result: {
      type: 'collapsible',
