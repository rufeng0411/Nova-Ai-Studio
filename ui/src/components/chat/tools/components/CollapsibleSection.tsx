...
export const CollapsibleSection: React.FC<CollapsibleSectionProps> = ({
 ...
 {toolName && (
 <span className="flex-shrink-0 font-medium text-gray-500">{toolName}</span>
 )}
 {toolName && (
 <span className="flex-shrink-0 text-[10px] text-gray-300">/</span>
 )}
 {onTitleClick ? (
 <button
 onClick={(e) => { e.preventDefault(); e.stopPropagation(); onTitleClick(); }}
 className="flex-1 truncate text-left font-mono text-blue-600 transition-colors hover:text-info hover:underline dark:text-blue-400 dark:hover:text-blue-300"
 >
 {title}
 </button>
 ) : (
