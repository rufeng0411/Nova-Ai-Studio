...
export const SubagentContainer: React.FC<SubagentContainerProps> = ({
 ...
 const description = parsedInput?.description || '子任务';
 ...
 const typeLabel = subagentType === 'general-purpose' ? '通用' : subagentType;
 const title = `子任务 / ${typeLabel}：${description}`;
 ...
 <span className="font-medium text-muted-foreground dark:text-gray-300">{currentTool.toolName}</span>
 ...
 <svg className="h-3 w-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0">
 ...
 <summary className="flex cursor-pointer items-center gap-1 text-[11px] text-gray-400 hover:text-muted-foreground dark:text-gray-500 dark:hover:text-gray-300">
 <svg
 ...
 <div className="mt-1 space-y-0.5 border-l border-border pl-3 dark:border-gray-700">
 {childTools.map((child, index) => (
 <div key={child.toolId} className="flex items-center gap-1.5 text-[11px] text-gray-500">
