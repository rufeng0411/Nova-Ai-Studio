...
export const AskUserQuestionPanel: React.FC<PermissionPanelProps> = ({
 ...
 const questions = normalizeElicitationQuestions(input?.questions) as Question[];
 ...
 const openText = (otherTexts.get(idx) || '').trim();
 if (openText) answers[q.question] = openText;
 return;
 }
 ...
 const complete = questions.every((question) => Boolean(answers[question.question]?.trim()));
 if (!complete) return;
 onDecision(request.requestId, { allow: true, updatedInput: { ...input, answers } });
 }, [onDecision, request.requestId, input, buildAnswers, questions]);
 ...
 const openEndedText = (otherTexts.get(currentStep) || '').trim();
 const hasCurrentSelection = isOpenEnded
   ? openEndedText.length > 0
   : selected.size > 0 || (isOtherOn && openEndedText.length > 0);
 const builtAnswers = buildAnswers();
 const allQuestionsAnswered = questions.every((question) => Boolean(builtAnswers[question.question]?.trim()));
 ...
 role={isOpenEnded ? undefined : (multi ? 'group' : 'radiogroup')}
 ...
 {!isOpenEnded ? q.options.map((opt, optIdx) => {
 ...
 disabled={isLast ? !allQuestionsAnswered : !hasCurrentSelection}
