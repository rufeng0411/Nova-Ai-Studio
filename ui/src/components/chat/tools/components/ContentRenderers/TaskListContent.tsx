...
import { localizeTodoStatus } from '../../../../../shared/localizeToolDisplayText';
...
const statusConfig = {
 completed: {
 icon: (
 <svg className="h-3.5 w-3.5 text-green-500 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0">
 ...
 in_progress: {
 icon: (
 <svg className="h-3.5 w-3.5 text-blue-500 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0">
 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
 </svg>
 ),
 textClass: 'text-foreground ',
 badgeClass: 'bg-blue-100 dark:bg-blue-900/30 text-info border-blue-200 dark:border-blue-800'
 },
 pending: {
 icon: (
 <svg className="h-3.5 w-3.5 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0">
 <circle cx="12" cy="12" r="9" strokeWidth={2} />
 </svg>
 ),
 textClass: 'text-foreground dark:text-gray-300',
 badgeClass: 'bg-muted dark:bg-card text-muted-foreground border-border dark:border-gray-700'
 }
};
...
export const TaskListContent: React.FC<TaskListContentProps> = ({ content }) => {
 ...
 {localizeTodoStatus(task.status, t)}
 </span>
 </div>
 );
 })}
