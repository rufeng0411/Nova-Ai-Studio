...
const STATUS_CONFIG: Record<TodoStatus, StatusConfig> = {
 ...
 textClassName: 'line-through text-gray-500 ',
 },
 in_progress: {
 icon: Clock,
 ...
 badgeClassName:
 'bg-muted dark:bg-card text-muted-foreground border-border dark:border-gray-700',
 textClassName: 'text-foreground ',
 },
};
...
const TodoRow = memo(
 ...
 return (
 <div className="flex items-start gap-2 rounded border border-border bg-card p-2 transition-colors dark:border-gray-700 dark:bg-card">
 ...
 >
 {localizeTodoStatus(todo.status, t)}
 </Badge>
 </div>
 </div>
 </div>
...
const TodoList = memo(
 ...
 {t('toolDisplay.todoListHeader', {
 count: normalizedTodos.length,
 defaultValue: `Todo list (${normalizedTodos.length} items)`,
 })}
 </div>
 )}
