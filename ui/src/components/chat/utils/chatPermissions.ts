...
/** Tools that must stay interactive — never auto-approve. */
export const INTERACTIVE_PERMISSION_TOOL_NAMES = new Set([
  'AskUserQuestion',
  'ask_user_question',
  'ExitPlanMode',
  'ExitPlanModeV2',
...
/** PD-SAAS-FORK: SaaS default skips per-tool banners — system auto-approves instead. */
export function shouldSilenceToolPermissionPrompt(): boolean {
  return getPilotDeckSettings().skipPermissions;
}

export function isToolPermissionFailure(message: ChatMessage | null | undefined): boolean {
  if (!message?.toolResult?.isError) return false;
  if (isPlanModeToolDeny(message)) return false;
  const errorCode = message.toolResult.errorCode;
  if (errorCode && PERMISSION_ERROR_CODES.has(errorCode)) return true;
  if (errorCode && !PERMISSION_ERROR_CODES.has(errorCode)) return false;
  const content = typeof message.toolResult.content === 'string'
    ? message.toolResult.content
    : '';
...
export function shouldAutoApproveSessionToolPermission(input: {
  toolName?: string;
  isElicitation?: boolean;
}): boolean {
  if (!shouldSilenceToolPermissionPrompt()) return false;
  if (input.isElicitation) return false;
  const toolName = String(input.toolName ?? '');
  return !INTERACTIVE_PERMISSION_TOOL_NAMES.has(toolName);
}
...
export function getPilotDeckPermissionSuggestion(
  ...
  if (shouldSilenceToolPermissionPrompt()) return null;
  ...
  // Already saved in allow-list — showing "grant again" is misleading; auto-recovery owns retry.
