...
import {
  ...
  getStreamContentKey,
  isScrollNearBottom,
  resolveInMemorySessionSwitchMode,
  resolveSessionLoadFlagsAfterTailHydrate,
...
describe('resolveInMemorySessionSwitchMode', () => {
  it('fetches when no warm slot exists', () => {
    expect(resolveInMemorySessionSwitchMode({ slot: null, isStale: false })).toBe('fetch');
    expect(resolveInMemorySessionSwitchMode({
      slot: { serverMessages: [], evicted: false },
      isStale: false,
    })).toBe('fetch');
  });

  it('instantly switches for fresh in-memory transcripts', () => {
    expect(resolveInMemorySessionSwitchMode({
      slot: { serverMessages: [{ id: 'm1' }], evicted: false },
      isStale: false,
    })).toBe('instant');
  });

  it('instantly switches when background realtime is still attached', () => {
    expect(resolveInMemorySessionSwitchMode({
      slot: {
        serverMessages: [{ id: 'm1' }],
        realtimeMessages: [{ id: 'rt1' }],
        evicted: false,
      },
      isStale: true,
    })).toBe('instant');
  ...
  it('revalidates stale warm slots in the background', () => {
    expect(resolveInMemorySessionSwitchMode({
      slot: { serverMessages: [{ id: 'm1' }], evicted: false },
      isStale: true,
    })).toBe('revalidate');
...
describe('resolveSessionLoadFlagsAfterTailHydrate', () => {
  it('uses instant cache with background revalidate when tail hydrate hits', () => {
    expect(resolveSessionLoadFlagsAfterTailHydrate(true)).toEqual({
      isLoadingSessionMessages: false,
      isRevalidatingSessionMessages: true,
    });
  });

  it('shows loading when tail cache misses', () => {
    expect(resolveSessionLoadFlagsAfterTailHydrate(false)).toEqual({
...
describe('session switch scroll restore', () => {
  ...
  it('restores mid-read position when returning to a session', () => {
    expect(resolveSessionScrollDecision(midReadBase)).toEqual({
      action: 'restore',
      scrollTop: 512,
    });
  });
  ...
  it('scrolls to bottom on first visit', () => {
    expect(resolveSessionScrollDecision({ ...midReadBase, hasEverVisited: false }).action).toBe('scrollBottom');
  });

  it('scrolls to bottom when user left near bottom', () => {
    expect(resolveSessionScrollDecision({ ...midReadBase, wasNearBottomAtLeave: true }).action).toBe('scrollBottom');
  });
});
