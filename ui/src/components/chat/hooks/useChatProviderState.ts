...
export function useChatProviderState({ selectedSession }: UseChatProviderStateArgs) {
  const [permissionMode, setPermissionModeState] = useState<PermissionMode>(() => {
    return readStoredPermissionMode(DEFAULT_PERMISSION_MODE_KEY) || 'bypassPermissions';
  });
  ...
  useEffect(() => {
    ...
    if (!selectedSession?.id) {
      setPermissionModeState(defaultMode || 'bypassPermissions');
      return;
    }

    const savedMode = readStoredPermissionMode(`permissionMode-${selectedSession.id}`);
    setPermissionModeState(savedMode || defaultMode || 'bypassPermissions');
  }, [selectedSession?.id]);
