...
import {
 AUTO_RECOVERY_CONTINUE_USER_SET_KEY,
...
import type { PermissionMode, PilotDeckSettings } from '../../../chat/types/types';
...
const PERMISSION_MODES: PermissionMode[] = ['default', 'bypassPermissions'];

function readStoredPermissionMode(): PermissionMode {
 const stored = safeLocalStorage.getItem(DEFAULT_PERMISSION_MODE_KEY);
 if (!stored || stored === 'default') return 'bypassPermissions';
 return PERMISSION_MODES.includes(stored as PermissionMode) ? (stored as PermissionMode) : 'bypassPermissions';
}
...
export default function PermissionsSettingsTab() {
 ...
 const [autoRecoveryContinue, setAutoRecoveryContinue] = useState(readDefaultAutoContinueEnabled);
 ...
 setAutoRecoveryContinue(Boolean(getPilotDeckSettings().autoRecoveryContinue));
 }, []);
 ...
 safeLocalStorage.setItem(DEFAULT_PERMISSION_MODE_KEY, mode);
 window.dispatchEvent(new Event('pilotdeck-settings-changed'));
 if (mode === 'bypassPermissions') {
 setSkipPermissions(true);
 persist({ skipPermissions: true });
 } else if (mode === 'default') {
 setSkipPermissions(false);
 persist({ skipPermissions: false });
 }
 };

 const handleAutoRecoveryContinueChange = (value: boolean) => {
 setAutoRecoveryContinue(value);
 persist({
   autoRecoveryContinue: value,
   [AUTO_RECOVERY_CONTINUE_USER_SET_KEY]: true,
 });
 };
 ...
 <SettingsSection
 title={t('permissions.chatBehaviorTitle', { defaultValue: '对话行为' })}
 description={t('permissions.chatBehaviorDescription', {
 defaultValue: '控制新对话默认如何请求权限，以及出错后是否自动尝试继续。',
 })}
 >
 <SettingsCard>
 <SettingsRow
 label={t('permissions.defaultPermissionMode', { defaultValue: '默认权限模式' })}
 description={t('permissions.defaultPermissionModeHint', {
 defaultValue: '新对话将按此模式运行；计划模式仍会单独要求确认。',
 })}
 >
 ...
 >
 {t('permissions.modeDefault', { defaultValue: '默认（需确认）' })}
 </Button>
 <Button
 type="button"
 ...
 >
 {t('permissions.modeFullAccess', { defaultValue: '完全访问' })}
 </Button>
 </div>
 </SettingsRow>
 <SettingsRow
 label={t('permissions.autoRecoveryContinue', {
 defaultValue: '出错后自动尝试继续（最多 3 次）',
 })}
 description={t('permissions.autoRecoveryContinueHint', {
 defaultValue: '暂停恢复时自动发送「继续」，无需手动输入。',
 })}
 >
 <SettingsToggle
 checked={autoRecoveryContinue}
 ariaLabel={t('permissions.autoRecoveryContinue', {
 defaultValue: '出错后自动尝试继续（最多 3 次）',
 })}
 onChange={handleAutoRecoveryContinueChange}
 />
 </SettingsRow>
 </SettingsCard>
 </SettingsSection>
 ...
 {skipPermissions ? (
 <div className="border-t border-border px-4 py-2.5 text-xs leading-relaxed text-amber-700">
