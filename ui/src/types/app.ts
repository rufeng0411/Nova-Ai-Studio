...
export type ProjectSessionKind = 'background_task' | 'n2_bot'; // PD-SAAS-FORK: n2_bot steward ≠ background_task

// >>> pilotdeck-fork: capability-hub
export type AppTab = 'home' | 'chat' | 'always-on' | 'files' | 'shell' | 'git' | 'tasks' | 'memory' | 'skills' | 'discover' | 'preview' | 'dashboard' | `plugin:${string}`;
// <<< pilotdeck-fork: capability-hub
...
export interface ProjectSession {
  ...
  executionStatus?: 'idle' | 'queued' | 'running' | 'paused';
...
export function getSessionRequestParams(
  session: ProjectSession | null | undefined,
): SessionRequestParams & { taskId?: string } {
...
export interface Project {
  ...
  fullPath: string;
  path?: string;
  /** PD-SAAS-FORK: gateway transcript key (canonical); may differ from fullPath when sync on. */
  sessionProjectKey?: string;
  storageKind?: 'local' | 'cloud' | 'legacy';
  syncEnabled?: boolean;
  writable?: boolean;
  workspaceUuid?: string;
  /** Client-only: keep in sidebar until GET /api/projects includes this row. */
