...
export function useDeviceSettings(options: UseDeviceSettingsOptions = {}) {
  ...
  useEffect(() => {
    ...
    const maxWidthQuery = window.matchMedia(`(max-width: ${mobileBreakpoint - 1}px)`);
    const onBreakpointChange = () => checkMobile();

    window.addEventListener('resize', checkMobile);
    window.visualViewport?.addEventListener('resize', checkMobile);

    if (typeof maxWidthQuery.addEventListener === 'function') {
      maxWidthQuery.addEventListener('change', onBreakpointChange);
    } else {
      maxWidthQuery.addListener(onBreakpointChange);
    }

    return () => {
      window.removeEventListener('resize', checkMobile);
      window.visualViewport?.removeEventListener('resize', checkMobile);
      if (typeof maxWidthQuery.removeEventListener === 'function') {
        maxWidthQuery.removeEventListener('change', onBreakpointChange);
      } else {
        maxWidthQuery.removeListener(onBreakpointChange);
      }
    };
