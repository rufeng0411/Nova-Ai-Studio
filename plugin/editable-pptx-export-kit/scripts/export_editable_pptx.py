...
def export_editable_pptx(
    ...
    # 根据实际图片尺寸动态设置幻灯片尺寸
    # 统一到最小尺寸，并检查所有图片是否为同一画面比例（相对容差，与 PPT 白名单一致）
    if editable_images:
        ASPECT_RATIO_TOLERANCE = 0.02  # 允许2%的相对误差

        ref_ratio = editable_images[0].width / editable_images[0].height
        min_width = float('inf')
        min_height = float('inf')

        for idx, img in enumerate(editable_images):
            aspect_ratio = img.width / img.height
            ratio_diff = abs(aspect_ratio - ref_ratio) / ref_ratio

            if ratio_diff > ASPECT_RATIO_TOLERANCE:
                logger.error(
                    f"图片 {idx + 1} ({image_paths[idx]}) 与首张画面比例不一致: "
                    f"{img.width}x{img.height} (比例 {aspect_ratio:.4f}, 参考 {ref_ratio:.4f})"
                )
                raise ValueError(
                    f"所有图片须为相同画面比例，但第 {idx + 1} 张 ({img.width}x{img.height}) 与首张不一致"
                )
