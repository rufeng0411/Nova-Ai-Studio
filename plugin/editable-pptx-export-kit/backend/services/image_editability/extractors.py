...
class MinerUElementExtractor(ElementExtractor):
    ...
    @staticmethod
    def _mineru_result_is_valid(result_dir: Path) -> bool:
        if not result_dir.is_dir():
            return False
        if not (result_dir / "layout.json").exists():
            return False
        return bool(list(result_dir.glob("*_content_list.json")))

    @staticmethod
    def _mineru_cache_key(image_path: Path) -> str:
        import hashlib
    ...
    def _mineru_cache_index_path(self, cache_key: str) -> Path:
        return self._upload_folder / "mineru_cache" / f"{cache_key}.json"

    def _find_cache(self, image_path: str) -> Optional[str]:
        """查找缓存的 MinerU 结果（按图片内容 hash，重复导出同页可跳过远程解析）。"""
        try:
            from utils.ppt_export_config import get_ppt_export_mineru_cache_enabled

            if not get_ppt_export_mineru_cache_enabled():
                return None
            ...
            cache_key = self._mineru_cache_key(img_path)
            index_path = self._mineru_cache_index_path(cache_key)
            ...
            with open(index_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            extract_id = (data.get("extract_id") or "").strip()
            if not extract_id:
                return None

            result_dir = (self._upload_folder / "mineru_files" / extract_id).resolve()
            if self._mineru_result_is_valid(result_dir):
                logger.info("MinerU 缓存命中: %s -> %s", img_path.name, extract_id)
    ...
    def _save_cache(self, image_path: str, extract_id: str) -> None:
        try:
            from utils.ppt_export_config import get_ppt_export_mineru_cache_enabled
            ...
            img_path = Path(image_path)
            if not img_path.exists() or not (extract_id or "").strip():
                return

            cache_key = self._mineru_cache_key(img_path)
            index_dir = self._upload_folder / "mineru_cache"
            ...
            logger.debug("MinerU 缓存已写入: %s -> %s", cache_key, extract_id)
        except Exception as e:
            logger.debug(f"写入 MinerU 缓存失败: {e}")
    ...
    def _parse_image(self, image_path: str, depth: int) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            self._save_cache(image_path, extract_id)
