        ...
        **kwargs
        ...
        Args:
            ...
            **kwargs: 其他配置参数
                ...
                - enhance_quality: 混合Inpaint是否启用画质提升（默认 False，见 PPT_EXPORT_INPAINT_ENHANCE_QUALITY）
        ...
        if has_app_context() and current_app:
            if mineru_token is None:
                ...
                from utils.mineru_config import effective_mineru_token

                mineru_token = effective_mineru_token(
                    current_app.config.get('MINERU_TOKEN'),
                    Config.MINERU_TOKEN,
                ) or (current_app.config.get('MINERU_TOKEN') or '')
        ...
        # 验证必需配置：纯 MinerU 必须有 Token；混合模式可在 Token 失效/缺失时降级百度 OCR
        if not (mineru_token and str(mineru_token).strip()):
            if use_hybrid_extractor:
                from services.ai_providers.ocr import create_baidu_accurate_ocr_provider
                if create_baidu_accurate_ocr_provider():
                    logger.warning("MINERU_TOKEN 未配置，混合提取将仅使用百度 OCR")
                    mineru_token = ""
                else:
                    raise ValueError(
                        "MinerU token is required when BAIDU_API_KEY is not configured. "
                        "Please configure MINERU_TOKEN or BAIDU_API_KEY."
                    )
            else:
        ...
        if effective_inpaint_method == 'hybrid':
            # 混合Inpaint提供者（百度修复 + 可选生成式画质提升）
            from utils.ppt_export_config import get_ppt_export_inpaint_enhance_quality

            enhance_quality = kwargs.get('enhance_quality', get_ppt_export_inpaint_enhance_quality())
            ...
            if hybrid_inpaint:
                ...
                logger.info(
                    "✅ 混合Inpaint提供者已创建（百度修复%s）",
                    " + 生成式画质提升" if enhance_quality else "，已跳过生成式画质提升以提速",
                )
            else:
                # 回退到纯生成式重绘
                generative_provider = InpaintProviderFactory.create_generative_edit_provider(
