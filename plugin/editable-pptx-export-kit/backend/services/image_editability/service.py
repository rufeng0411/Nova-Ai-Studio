...
class ImageEditabilityService:
    ...
    def make_image_editable(
        ...
        # 检查提取是否有错误（根层级：无元素才失败；部分失败可降级）
        if extraction_result.has_error and depth == 0 and not extraction_result.elements:
            raise RuntimeError(f"版面分析失败: {extraction_result.error}")
        if extraction_result.has_error and depth == 0 and extraction_result.elements:
            logger.warning(
                "版面分析部分失败，已使用可用元素继续: %s",
        ...
        text_like = sum(
            1 for e in elements
            if (getattr(e, 'element_type', None) in ('text', 'title', 'list', 'paragraph', 'header', 'footer', 'heading', 'table_caption', 'image_caption', 'table_cell')
                and (getattr(e, 'content', None) or '').strip())
        )
        logger.info(
            f"{'  ' * depth}[{image_id}] 提取到 {len(elements)} 个元素, 文本类={text_like}, size={width}x{height}, path={image_path}"
        )
