...
class ExportError(Exception):
    ...
    def _get_default_help_text(self, error_type: str) -> str:
        ...
        help_texts = {
            ...
            'no_text_layers': '请检查 MINERU_TOKEN、百度 OCR 等版面分析配置；导出设置建议使用「混合提取」。若仅需整页配图，请使用「快速 PPTX」导出而非可编辑导出。',
        }
...
def _env_int(name: str, default: int, min_v: int = 1, max_v: int = 16384) -> int:
...
def _pdf_export_skip_recompress() -> bool:
    return os.environ.get("PPT_EXPORT_PDF_NO_RECOMPRESS", "").strip().lower() in ("1", "true", "yes", "on")


def _pptx_export_skip_recompress() -> bool:
    return os.environ.get("PPT_EXPORT_PPTX_NO_RECOMPRESS", "").strip().lower() in ("1", "true", "yes", "on")


def get_pptx_export_embed_settings() -> Tuple[int, int, bool]:
    """Return (max_long_edge, jpeg_quality, skip_recompress) for PPTX embed."""
    max_long_edge = _env_int("PPT_EXPORT_PPTX_MAX_LONG_EDGE", 1920, 640, 8192)
    jpeg_quality = _env_int("PPT_EXPORT_PPTX_JPEG_QUALITY", 82, 40, 100)
    return max_long_edge, jpeg_quality, _pptx_export_skip_recompress()


def prepare_image_for_embed(
    src_path: str,
    max_long_edge: int,
    jpeg_quality: int,
    ...
    """
    将图片压缩为 JPEG 供 PPTX/PDF 嵌入，显著减小体积；不改变幻灯片上 shape 的英寸尺寸。
    若 dest_path 未指定则写入临时文件（调用方需在导出完成后删除或由 ExportEmbedImageCache 管理）。
    """
    if dest_path is None:
        fd, dest_path = tempfile.mkstemp(suffix=".jpg", prefix="ppt_export_embed_")
        os.close(fd)
    _write_pdf_slide_jpeg(src_path, dest_path, max_long_edge, jpeg_quality)
    return dest_path


class ExportEmbedImageCache:
    """导出期间缓存 JPEG 压缩副本，避免同一源图重复压缩；退出时清理临时目录。"""
    ...
    def __init__(
        ...
        self.skip_recompress = skip_recompress
        self._tmpdir = tempfile.TemporaryDirectory(prefix="ppt_export_embed_")
        self._cache: Dict[str, str] = {}
        self._counter = 0

    def prepare(self, src_path: str) -> str:
        if self.skip_recompress or not src_path:
            return src_path
        if not os.path.isfile(src_path):
            return src_path
        cached = self._cache.get(src_path)
        if cached:
            return cached
        self._counter += 1
        dest = os.path.join(self._tmpdir.name, f"embed_{self._counter:05d}.jpg")
        prepare_image_for_embed(src_path, self.max_long_edge, self.jpeg_quality, dest_path=dest)
        self._cache[src_path] = dest
        return dest

    def __enter__(self) -> "ExportEmbedImageCache":
        return self
...
def _decode_resize_rgb_for_pdf(src_path: str, max_long_edge: int) -> Image.Image:
    """
    打开幻灯片配图，归一为 RGB、按长边上限缩小，便于后续 JPEG 嵌入 PDF，显著减小体积。
    """
    ...
    w, h = im.size
    if w <= 0 or h <= 0:
        raise ValueError(f"invalid image dimensions: {src_path}")
    if max(w, h) > max_long_edge:
        if w >= h:
            nw = max_long_edge
            nh = max(1, int(round(h * max_long_edge / float(w))))
        else:
            nh = max_long_edge
            nw = max(1, int(round(w * max_long_edge / float(h))))
        im = im.resize((nw, nh), Image.Resampling.LANCZOS)
    return im


def _write_pdf_slide_jpeg(src_path: str, dest_path: str, max_long_edge: int, jpeg_quality: int) -> None:
    """将单页图写成 JPEG，供 img2pdf 无损嵌入 PDF 流（体积小）。"""
    rgb = _decode_resize_rgb_for_pdf(src_path, max_long_edge)
    rgb.save(
        dest_path,
        format="JPEG",
...
def _letterbox_picture_inches(
    image_path: str,
    slide_w_in: float,
    slide_h_in: float,
) -> Optional[Tuple[float, float, float, float]]:
    """
    按比例将图片装入幻灯片，避免强制拉伸导致版式/图内文字变形。
    返回 (left_in, top_in, pic_w_in, pic_h_in)；无法读取图片时返回 None。
    """
    try:
        with Image.open(image_path) as im:
            iw, ih = im.size
    except Exception as e:
        logger.warning("无法读取图片尺寸，将退化为拉伸铺满: %s — %s", image_path, e)
        return None
    if iw <= 0 or ih <= 0 or slide_w_in <= 0 or slide_h_in <= 0:
        return None
    img_ratio = iw / ih
    slide_ratio = slide_w_in / slide_h_in
    if img_ratio > slide_ratio:
        pic_w = slide_w_in
        pic_h = slide_w_in / img_ratio
    else:
        pic_h = slide_h_in
        pic_w = slide_h_in * img_ratio
    left = (slide_w_in - pic_w) / 2.0
    top = (slide_h_in - pic_h) / 2.0
...
class ExportService:
    ...
    def create_pptx_from_images(image_paths: List[str], output_file: str = None, aspect_ratio: str = '16:9') -> bytes:
        ...
        """
        max_long_edge, jpeg_quality, skip_recompress = get_pptx_export_embed_settings()

        with ExportEmbedImageCache(max_long_edge, jpeg_quality, skip_recompress) as embed_cache:
            prs = Presentation()
            ...
            for image_path in image_paths:
                ...
                embed_path = embed_cache.prepare(image_path)
                blank_slide_layout = prs.slide_layouts[6]
                slide = prs.slides.add_slide(blank_slide_layout)

                box = _letterbox_picture_inches(embed_path, page_w, page_h)
                if box is not None:
                    left_in, top_in, pic_w_in, pic_h_in = box
                    slide.shapes.add_picture(
                        embed_path,
                        left=Inches(left_in),
                        top=Inches(top_in),
                        width=Inches(pic_w_in),
                        height=Inches(pic_h_in),
                    )
                else:
                    slide.shapes.add_picture(
                        embed_path,
            ...
            if output_file:
                ...
                if os.path.isfile(output_file):
                    logger.info(
                        "[export_pptx] saved pages=%s bytes=%s recompress=%s max_long_edge=%s q=%s path=%s",
                        len(image_paths),
            ...
            logger.info(
                "[export_pptx] generated pages=%s bytes=%s recompress=%s max_long_edge=%s q=%s",
                len(image_paths),
                len(data),
                not skip_recompress,
                max_long_edge,
    ...
    def create_pdf_from_images(image_paths: List[str], output_file: str = None, aspect_ratio: str = '16:9') -> Optional[bytes]:
        ...
        默认在嵌入前将每页转为 JPEG（限制长边、适度质量），显著减小 PDF 体积；img2pdf 将 JPEG 以 DCT 流嵌入，不再二次无损膨胀。

        环境变量（可选，见 .env.example）：
            PPT_EXPORT_PDF_MAX_LONG_EDGE — 长边像素上限，默认 1920（范围约 640–8192）
            PPT_EXPORT_PDF_JPEG_QUALITY — JPEG 质量 40–100，默认 82
            PPT_EXPORT_PDF_NO_RECOMPRESS — 设为 1/true/yes/on 时仍按原图路径直嵌（旧行为、文件往往更大）

        Args:
            image_paths: List of absolute paths to images
            output_file: Optional output file path (if None, returns bytes)
            aspect_ratio: 页面宽高比字符串，与幻灯片一致
        ...
        max_long_edge = _env_int("PPT_EXPORT_PDF_MAX_LONG_EDGE", 1920, 640, 8192)
        jpeg_quality = _env_int("PPT_EXPORT_PDF_JPEG_QUALITY", 82, 40, 100)
        skip_recompress = _pdf_export_skip_recompress()

        try:
            if skip_recompress:
                logger.info(
                    "Using img2pdf for PDF export (%s pages, original images, PPT_EXPORT_PDF_NO_RECOMPRESS set)",
                    len(valid_paths),
                )
                paths_for_pdf = valid_paths
                page_w, page_h = _get_page_size_inches(aspect_ratio)
                layout_fun = img2pdf.get_layout_fun(
                    pagesize=(img2pdf.in_to_pt(page_w), img2pdf.in_to_pt(page_h)),
                    fit=img2pdf.FitMode.exact,
                )
                pdf_bytes = img2pdf.convert(paths_for_pdf, layout_fun=layout_fun)
            else:
                logger.info(
                    "Using img2pdf for PDF export (%s pages, JPEG recompress max_long_edge=%s q=%s)",
                    len(valid_paths),
                    max_long_edge,
                    jpeg_quality,
                )
                with tempfile.TemporaryDirectory(prefix="pdf_export_") as tmpdir:
                    paths_for_pdf = []
                    for i, p in enumerate(valid_paths):
                        dest = os.path.join(tmpdir, f"slide_{i:05d}.jpg")
                        _write_pdf_slide_jpeg(p, dest, max_long_edge, jpeg_quality)
                        paths_for_pdf.append(dest)
                    page_w, page_h = _get_page_size_inches(aspect_ratio)
                    layout_fun = img2pdf.get_layout_fun(
                        pagesize=(img2pdf.in_to_pt(page_w), img2pdf.in_to_pt(page_h)),
                        fit=img2pdf.FitMode.exact,
            ...
            if output_file:
                with open(output_file, "wb") as f:
                    f.write(pdf_bytes)
                logger.info(
                    "[export_pdf] saved pages=%s bytes=%s recompress=%s max_long_edge=%s q=%s path=%s",
                    len(valid_paths),
                    len(pdf_bytes),
                    not skip_recompress,
                    max_long_edge,
                ...
                return None
            logger.info(
                "[export_pdf] generated pages=%s bytes=%s recompress=%s max_long_edge=%s q=%s",
                len(valid_paths),
                len(pdf_bytes),
                not skip_recompress,
                max_long_edge,
            ...
            return ExportService.create_pdf_from_images_pillow(
                valid_paths, output_file, aspect_ratio,
    ...
    def create_pdf_from_images_pillow(
        ...
        *,
        ...
        Args:
            image_paths: List of absolute paths to images
            output_file: Optional output file path (if None, returns bytes)
            max_long_edge / jpeg_quality / skip_recompress: 与主路径一致，用于缩小嵌入体积
        ...
        for image_path in image_paths:
            ...
            if skip_recompress:
                ...
                img = _decode_resize_rgb_for_pdf(image_path, max_long_edge)
    ...
    _TEXT_LIKE_ELEMENT_TYPES = frozenset({
        'text', 'title', 'list', 'paragraph', 'header', 'footer', 'heading',
        'table_caption', 'image_caption',
    })

    @staticmethod
    def _count_text_like_elements(elements: List) -> int:
        """统计将被导出为可编辑文本框的元素数量（含表格单元格与递归子元素）。"""
        n = 0
        for elem in elements:
            t = elem.element_type
            content = (elem.content or '').strip()
            if t in ExportService._TEXT_LIKE_ELEMENT_TYPES and content:
                n += 1
            elif t == 'table_cell' and content:
                n += 1
            elif t == 'table' and getattr(elem, 'children', None):
                n += ExportService._count_text_like_elements(elem.children)
            elif t in ('image', 'figure', 'chart') and getattr(elem, 'children', None):
                n += ExportService._count_text_like_elements(elem.children)
        return n
    ...
    def create_editable_pptx_with_recursive_analysis(
        ...
        if editable_images is not None:
            ...
            from utils.ppt_export_config import get_ppt_export_inpaint_enhance_quality
            import time
            ...
            config = ServiceConfig.from_defaults(
                ...
                enhance_quality=get_ppt_export_inpaint_enhance_quality(),
            )
            ...
            layout_elapsed = time.monotonic() - layout_t0
            logger.info(
                "[export_editable_pptx] layout_analysis done pages=%s workers=%s elapsed_sec=%.1f "
                "enhance_quality=%s",
                total_pages,
                max_workers,
                layout_elapsed,
        ...
        # 2.3 若完全没有可导出文本层，继续生成则与「整页配图」无异
        total_text_layers = sum(
            ExportService._count_text_like_elements(img.elements) for img in editable_images
        )
        if total_text_layers == 0:
            msg = (
                "版面分析未识别到任何可编辑文本：生成的 PPTX 每页仅有一张整页背景图（无可选中的文字框）。"
                "请检查 MINERU_TOKEN 是否有效、混合模式下的百度 OCR 是否已配置；"
                "导出设置建议使用「混合提取」。若只需要每页一张成图，请使用「快速 PPTX」导出。"
            )
            logger.error(
                "[export_editable_pptx] no_text_layers slides=%s extractor=%s inpaint=%s fail_fast=%s",
                len(editable_images),
            ...
            if fail_fast:
                raise ExportError(
                    message=msg,
                    error_type='no_text_layers',
                    details={'slides': len(editable_images), 'text_layers': 0},
                )
        ...
        with ExportEmbedImageCache(max_long_edge, jpeg_quality, skip_recompress) as embed_cache:
            builder = PPTXBuilder()
            builder.embed_image_cache = embed_cache
            ...
            for page_idx, editable_img in enumerate(editable_images):
                ...
                if editable_img.clean_background and os.path.exists(editable_img.clean_background):
                    bg_path = embed_cache.prepare(editable_img.clean_background)
                    logger.info(f"    添加clean background: {bg_path}")
                    try:
                        slide.shapes.add_picture(
                            bg_path,
                            left=0,
                    ...
                    bg_path = embed_cache.prepare(editable_img.image_path)
                    logger.info(f"    使用原图作为背景: {bg_path}")
                    try:
                        slide.shapes.add_picture(
                            bg_path,
                            left=0,
                ...
                logger.info(
                    f"    元素数量: {len(editable_img.elements)}, 图片尺寸: {editable_img.width}x{editable_img.height}, "
            ...
            if output_file:
                ...
                if os.path.isfile(output_file):
                    logger.info(
                        "[export_editable_pptx] saved pages=%s bytes=%s recompress=%s max_long_edge=%s q=%s path=%s",
                        total_pages,
                        os.path.getsize(output_file),
                        not skip_recompress,
                        max_long_edge,
            ...
            logger.info(
                "[export_editable_pptx] generated pages=%s bytes=%s recompress=%s max_long_edge=%s q=%s",
                total_pages,
                len(pptx_bytes),
                not skip_recompress,
                max_long_edge,
