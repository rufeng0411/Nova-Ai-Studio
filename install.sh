...
# Nova Ai-Studio one-line installer for macOS and Linux.
# Usage (set your delivery git remote first):
#   NOVA_REPO_URL=https://your-host/your-org/nova-ai-studio.git curl -fsSL https://your-host/install.sh | bash
# Cloud bundle install: see scripts/release/remote-install.sh

REPO_URL="${NOVA_REPO_URL:-${PILOTDECK_REPO_URL:-}}"
...
if [[ -z "$REPO_URL" ]]; then
  fail "Set NOVA_REPO_URL (or PILOTDECK_REPO_URL) to your Nova Ai-Studio git remote before running this installer."
fi
...
install_git_lfs() {
  if [[ "$PLATFORM" == "macos" ]] && command -v brew >/dev/null 2>&1; then
    ...
    fail "git-lfs is required for Nova Ai-Studio assets. On macOS, install Homebrew and run: brew install git-lfs"
  fi
}
...
echo ""
echo -e "${BOLD}Nova Ai-Studio Installer${RESET}"
echo "====================="
echo ""
...
echo -e "Installing Nova Ai-Studio to ${DIM}${INSTALL_DIR}${RESET} ..."
install_or_update_repo
ensure_lfs_assets
echo ""
...
if has_playwright_chrome_for_testing; then
  ...
  if run_with_timeout "${BROWSER_INSTALL_TIMEOUT}" npx @playwright/mcp install-browser chrome-for-testing </dev/null; then
    ...
    warn "Nova Ai-Studio core features are still available."
...
if [[ "$COMMAND" == "help" ]]; then
  cat <<HELP
pilotdeck - start the Nova Ai-Studio web UI
  ...
  if [[ ":$PATH:" != *":$LOCAL_BIN:"* ]]; then
    ...
    if [[ -n "$SHELL_RC" ]]; then
      if [[ ! -f "$SHELL_RC" ]] || ! grep -qF '.local/bin' "$SHELL_RC" 2>/dev/null; then
        printf '\n# Added by Nova Ai-Studio installer\n%s\n' "$PATH_LINE" >> "$SHELL_RC"
