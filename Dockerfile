...
ARG PILOTDECK_SAAS_MODE=1
ENV PILOTDECK_SAAS_MODE=${PILOTDECK_SAAS_MODE}
ENV PILOTDECK_DISABLE_LOCAL_AUTH=0

# Build gateway (TypeScript → dist/)
# Skip bootstrap (skills already in image); ensure docs/ for prebuild template catalog write
ENV PILOTDECK_SKIP_BOOTSTRAP=1
RUN mkdir -p docs
RUN npm run build

# UI 在宿主机 pack.mjs 中预先 vite build（避免 pnpm 在容器内 hoist 问题）
...
# PD-SAAS-FORK: python3-pptx + playwright chromium for export_document / document OCR
RUN set -eux; \
    for attempt in 1 2 3 4 5; do \
      apt-get update && apt-get install -y --no-install-recommends \
        ripgrep git curl procps python3 python3-pip && break; \
      echo "apt install retry ${attempt}/5"; sleep 20; \
    done; \
    rm -rf /var/lib/apt/lists/*; \
    pip3 install --no-cache-dir --break-system-packages python-pptx Pillow requests; \
...
# PD-SAAS-FORK: Playwright browser for export_document HTML→PDF
...
COPY --from=builder /build/ui/src/shared/ ui/src/shared/
