...
| P0-1 fetch failed 对用户隐形并自动续跑 | 失败 | `npx vitest run src/saas/userActionBlocker.test.ts src/saas/resilience/stabilityFlags.test.ts` |
...
| P1-A 计划账本（按契约派生有序步骤） | 通过 | `npx vitest run src/agent/loop/planLedger.test.ts` |
| P1-D 质量验收（表列一致 + 结构空壳，软展示） | 通过 | `npx vitest run src/saas/final-acceptance/qualityChecks.test.ts` |
| P1-F 单工具超时看门狗（按工具 deadline，非破坏性） | 通过 | `npx vitest run src/agent/loop/toolWatchdog.test.ts` |
| P1-C2 流式退化截断（高风险，默认 OFF） | 通过 | `npx vitest run src/model/streaming/streamDegenerationGuard.test.ts` |
| P2-E in-turn 上下文压缩（折叠旧大 tool_result，可重读） | 通过 | `npx vitest run src/agent/loop/toolResultCompaction.test.ts` |
| P2 校验子代理决策（仅通过时独立复核，预算内） | 通过 | `npx vitest run src/agent/loop/verificationPass.test.ts` |
| /goal Feature 2 自然语言完成条件入 TaskGoalContract（N 轮内停 + 完成条件） | 失败 | `npx vitest run src/saas/taskState/taskGoalContract.test.ts src/saas/resilience/stabilityFlags.test.ts` |
| /goal Feature 1 观测态 LLM 二次复核（fail-open、不触发修复、不弹气泡、无路径遥测） | 通过 | `npx vitest run src/agent/loop/verificationReviewer.test.ts src/agent/loop/verificationPass.test.ts` |
