...
| 北京 AI 调研报告长任务 recovery | 失败 | `npx vitest run tests/saas/task-continuation-policy.test.ts tests/saas/final-acceptance.test.ts` |
...
| 文档导入、OCR、导出链路中断 | 失败 | `npx vitest run tests/saas/task-continuation-policy.test.ts` |
...
| 第三方 skill 缺配置 | 失败 | `npx vitest run tests/saas/task-continuation-policy.test.ts src/saas/taskState/taskExecutionStateMachine.test.ts` |
