        ...
        try:
            ...
            for attempt in range(4):
                if attempt > 0:
                    ...
                    logger.warning(f"⏳ 百度 OCR QPS/限流，{wait_s}s 后重试 ({attempt + 1}/4)")
                    time.sleep(wait_s)
                ...
                last_error = Exception(f"Baidu API error [{error_code}]: {error_msg}")
                # 18 = QPS limit; retry with backoff
                if error_code in (18, 19):
                    continue
                logger.error(f"❌ 百度API错误: [{error_code}] {error_msg}")
                raise last_error
            if result is None or 'error_code' in result:
                if last_error:
                    raise last_error
                raise Exception("Baidu OCR failed without response")
...
def create_baidu_accurate_ocr_provider(
    ...
    """Create Baidu accurate OCR provider from explicit token or API Key + Secret env."""
    token = (api_key or get_baidu_access_token() or os.environ.get("BAIDU_OCR_ACCESS_TOKEN") or os.environ.get("BAIDU_API_KEY") or "").strip()
    if not token:
        logger.warning("Baidu OCR access token not configured")
        return None
    return BaiduAccurateOCRProvider(token)
