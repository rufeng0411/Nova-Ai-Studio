...
// PD-SAAS-FORK: force-refresh integrated L2 skills on each bootstrap
const REFRESH_SKILL_SLUGS = new Set([
  'yixiaoer',
  'anth-docx',
  'pd-geo',
...
schemaVersion: 1
# PD-SAAS-FORK: Nova 新装默认中文（界面与 Agent 工作语言）
language: zh-CN
alwaysOn:
  enabled: false
  language: zh-CN
...
router:
  ...
  autoOrchestrate:
    ...
    allowedTools:
      ...
      - read_skill
      - yixiaoer_api
      - scaffold_mobile_mockup
      - write_file
      - edit_file
    subagentMaxTokens: 256000
  stats:
    enabled: true
cron:
  enabled: true
...
function syncRepoSkillsToPilotHome(pilotHome) {
  ...
  for (const sourceDir of skillDirs) {
    ...
    if (existsSync(targetPath)) {
      if (REFRESH_SKILL_SLUGS.has(slug)) {
...
const ORCH_PATCH_TOOLS = ['yixiaoer_api', 'scaffold_mobile_mockup', 'write_file', 'edit_file'];

function patchOrchestratorAllowedTools(configPath) {
  let content;
  try {
  ...
  const missing = ORCH_PATCH_TOOLS.filter((tool) => !new RegExp(`^\\s*-\\s*${tool}\\s*$`, 'm').test(content));
  if (missing.length === 0) return false;
  const block = content.match(/(\n    allowedTools:\n(?:      - .+\n)+)/);
  if (!block) return false;
  const insert = missing.map((tool) => `      - ${tool}\n`).join('');
  const next = content.replace(block[1], `${block[1]}${insert}`);
  try {
    writeFileSync(configPath, next, 'utf8');
    console.log(`[pilotdeck] Patched autoOrchestrate.allowedTools (+${missing.join(', ')}) in ${configPath}`);
    return true;
  } catch (error) {
    console.warn(
      `[pilotdeck] Could not patch allowedTools in ${configPath}: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
    return false;
...
function main() {
  ...
  if (existsSync(configPath)) {
    ...
    patchOrchestratorAllowedTools(configPath);
    return;
  }
