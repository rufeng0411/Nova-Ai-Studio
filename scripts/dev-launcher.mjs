#!/usr/bin/env node
/**
 * Standalone (OSS) dev launcher — use `npm run dev:standalone`.
 * Default `npm run dev` starts SaaS via dev-saas.mjs.
 *
 * Probe the three dev ports (server / gateway / vite), find the
...
async function main() {
  ...
  console.log('');
  console.log('[dev-launcher] browser URLs (use LAN IP on other devices):');
  for (const entry of formatLanUrls({ vitePort: vite.port, serverPort: server.port })) {
  ...
  const stack = await runDevStack(repoRoot, env);
