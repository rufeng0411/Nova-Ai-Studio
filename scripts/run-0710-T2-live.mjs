#!/usr/bin/env node
/**
 * PD-SAAS-FORK: 0710-T2 十案典型错误实机 Playwright（admin，workers=1 串行）。
 * 保留全部测试记录；跑完后自动深度分析。
 */
...
const spec = 'ui/e2e/saas/0710-T2-live.spec.ts';
const artifactRoot = path.join(root, 'artifacts/0710-T2');
const projectName = '0710-T2';
...
function startDevIfNeeded() {
  return new Promise((resolveStart) => {
    probe(baseUrl).then(async (ok) => {
      ...
      console.log('[0710-T2] starting dev:saas…');
...
async function ensureProject() {
  ...
  if (existing?.name) {
    console.log(`[0710-T2] reuse project → ${existing.name}`);
  ...
  console.log(`[0710-T2] created project → ${slug}`);
  return slug;
}
...
async function main() {
  ...
  if (!ready) {
    console.error(`[0710-T2] dev not ready at ${baseUrl} / ${serverUrl}`);
    process.exit(1);
  }
  console.log(`[0710-T2] dev ready → ${baseUrl}`);
  ...
  try {
    ...
    console.warn('[0710-T2] pre-create project failed:', err);
  }
  ...
  if (process.env.O710_T2_CASES) env.O710_T2_CASES = process.env.O710_T2_CASES;
  if (process.env.O710_T2_FRESH) env.O710_T2_FRESH = process.env.O710_T2_FRESH;
  ...
  console.log('[0710-T2] running deep analysis…');
  const analyze = spawnSync('node', ['scripts/analyze-0710-T2-live.mjs'], {
    cwd: root,
    stdio: 'inherit',
    shell: true,
  });
  ...
  console.log(`[0710-T2] finished exit=${code}`);
  console.log('[0710-T2] logs: artifacts/0710-T2/logs/');
  console.log('[0710-T2] report: docs/0710-T2-live-report-*.zh-CN.md');
  console.log('[0710-T2] analysis: docs/0710-T2-live-analysis-*.zh-CN.md');
  process.exit(code);
}

main().catch((err) => {
  console.error('[0710-T2] fatal', err);
  process.exit(1);
});
