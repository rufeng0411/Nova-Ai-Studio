#!/usr/bin/env node
/**
 * PD-SAAS-FORK: 0710-T1 十案实跑结果深度分析 + STDA/四线统一 + 优化建议。
 */
...
const manifestPath = path.join(root, 'tests/fixtures/goal-loop/0710-T1/cases.json');
...
function loadResults() {
  ...
  if (results.length === 0 && fs.existsSync(path.join(logDir, '0710-T1-live.jsonl'))) {
    for (const line of fs.readFileSync(path.join(logDir, '0710-T1-live.jsonl'), 'utf8').split(/\r?\n/).filter(Boolean)) {
      try {
...
function buildRecommendations(results, patterns) {
  ...
  recs.push({
    level: 'P2',
    title: '单案重跑与人工复查',
    detail: '侧栏打开项目 0710-T1，逐条 session URL 核对 Dock/文件夹/正文链接；JSONL 在 `artifacts/0710-T1/logs/`。',
  });
...
function main() {
  ...
  if (results.length === 0) {
    console.error('[analyze-0710-T1] no result files in', logDir);
    process.exit(1);
  }

  const analysisPath = path.join(root, 'docs', `0710-T1-live-analysis-${dateTag}.zh-CN.md`);
  fs.writeFileSync(analysisPath, `${buildAnalysisDoc(results)}\n`, 'utf8');
  console.log(`[analyze-0710-T1] wrote ${analysisPath} (${results.length} cases)`);
}
