...
class PPTXBuilder:
    ...
    def add_text_element(
        ...
        # Make title text bold (legacy behavior) unless style already set
        if (text_level == 1 or text_level == 'title') and not (text_style and getattr(text_style, 'is_bold', False)):
            is_bold = True
        
        font_name = getattr(text_style, 'font_name', None) if text_style else None

        def _apply_font_name(target) -> None:
            if font_name:
                try:
    ...
    def add_image_element(
        ...
        cache = getattr(self, 'embed_image_cache', None)
        if cache is not None:
            embed_path = cache.prepare(image_path)
        ...
        try:
            # Add image
            slide.shapes.add_picture(embed_path, left, top, width, height)
            logger.debug(f"Added image: {embed_path} at bbox {bbox}")
        except Exception as e:
